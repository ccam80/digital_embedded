...
//
// AndDefinition has `models.digital.executeFn = executeAnd` and no analog
...
// ===========================================================================
// 2-input And — full truth table at bitWidth=1
// ===========================================================================

describe("And 2-input bitWidth=1 (Cat 9)", () => {
...
// ===========================================================================
// 3-input And — covers wider input-count code path
// ===========================================================================

describe("And 3-input bitWidth=1 (Cat 9)", () => {
  function build3(facade: DefaultSimulatorFacade) {
    return facade.build({
      components: [
        ...
        { id: "g",   type: "And", props: { inputCount: 3, bitWidth: 1 } },
        { id: "out", type: "Out", props: { label: "OUT", bitWidth: 1 } },
      ],
      connections: [
        ["a:out", "g:In_1"],
  ...
  it("a single zero input forces output to 0", () => {
    const facade = new DefaultSimulatorFacade(registry);
    const coord = facade.compile(build3(facade));
    facade.setSignal(coord, "A", 1);
    facade.setSignal(coord, "B", 0);
...
// ===========================================================================
// Multi-bit And — bitwise AND across full bus width
// ===========================================================================

describe("And 2-input bitWidth=8 (Cat 9 multi-bit)", () => {
  function build8(facade: DefaultSimulatorFacade) {
    return facade.build({
      components: [
        { id: "a",   type: "In",  props: { label: "A", bitWidth: 8 } },
        { id: "b",   type: "In",  props: { label: "B", bitWidth: 8 } },
        { id: "g",   type: "And", props: { inputCount: 2, bitWidth: 8 } },
  ...
  it("0xFF AND 0x0F = 0x0F", () => {
    ...
    facade.step(coord);
    expect(facade.readSignal(coord, "OUT")).toBe(0x0F);
  });

  it("0xAA AND 0x55 = 0x00 (no overlapping bits)", () => {
    const facade = new DefaultSimulatorFacade(registry);
    const coord = facade.compile(build8(facade));
  ...
  it("0xFF AND 0xFF = 0xFF (all-ones bus)", () => {
    const facade = new DefaultSimulatorFacade(registry);
    const coord = facade.compile(build8(facade));
    facade.setSignal(coord, "A", 0xFF);
