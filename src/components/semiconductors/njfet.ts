  ...
  return {
    ...
    load(ctx: LoadContext): void {
      ...
      if (primedFromJct) {
        ...
        // jfetload.c:230-242: DEVfetlim — three-zone gate-threshold limiter.
        // ngspice uses the identical routine shared with MOSFET in
        // devsup.c.  Inlined here so the N-channel file stays self-contained
        // per spec §4 D-10 (no shared helper).
        {
          const vto_local = vto;
      ...
      // the "prime" nodes collapse to the external pins. Stamp gdpr/gspr
      // as self-terminal only when nonzero.
