...
function resolveParams(raw: MosfetParams, _kpDefault: number): ResolvedMosfetParams {
  const p: ResolvedMosfetParams = {
    ...
    drainSquares:  raw.drainSquares  ?? 1,
    sourceSquares: raw.sourceSquares ?? 1,
  };
...
export function createMosfetElement(
  ...
  const rawParams: MosfetParams = {
    ...
    RD: props.getModelParam<number>("RD"), RS: props.getModelParam<number>("RS"),
    ...
    drainSquares:  props.getModelParam<number>("drainSquares"),
  ...
  return {
     ...
     *
     * Uses ngspice's VBS / VBD convention verbatim. `vbs = vb - vs` and
     * `vbd = vb - vd` (mos1load.c:231-247).
     *
    ...
    load(ctx: LoadContext): void {
      ...
      // MODETRAN|MODETRANOP per mos1load.c:329-338).
      ...
      if (simpleGate) {
        ...
        if (!bypassed) {
          ...
          // pass through fetlim/limvds/pnjlim per mos1load.c:356-406, but MODE
      ...
      // Hoisted cap totals (populated above by bypass branch or below by Meyer block).
