...
describe("TLine", () => {
  describe("lossless_case", () => {
    it("lossless line: R_seg and G_seg stamps are zero when loss=0", () => {
      ...
      expect(TransmissionLineDefinition.getInternalNodeCount!(props)).toBe(4);
  ...
  describe("internal_node_count", () => {
    it("getInternalNodeCount returns (N-1)*2", () => {
      ...
      expect(TransmissionLineDefinition.getInternalNodeCount!(props)).toBe(18);

      props.set("segments", 2);
      expect(TransmissionLineDefinition.getInternalNodeCount!(props)).toBe(2);

      props.set("segments", 5);
      expect(TransmissionLineDefinition.getInternalNodeCount!(props)).toBe(8);
    });
  });
...
describe("TransmissionLine", () => {
  describe("definition", () => {
    ...
    it("has engineType 'analog'", () => {
      expect(TransmissionLineDefinition.engineType).toBe("analog");
    });

    it("has analogFactory", () => {
      expect(TransmissionLineDefinition.analogFactory).toBeDefined();
    });

    it("requires branch row", () => {
      expect(TransmissionLineDefinition.requiresBranchRow).toBe(true);
    });

    it("has getInternalNodeCount", () => {
      expect(TransmissionLineDefinition.getInternalNodeCount).toBeDefined();
    });
