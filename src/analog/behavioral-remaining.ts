...
// ---------------------------------------------------------------------------
// Switch analog factory — variable resistance (R_on / R_off)
//
// Pin nodeIds order (SPST, 1 pole):
//   nodeIds[0] = A1
//   nodeIds[1] = B1
//
// The closed/open state is read from props "closed" (default false).
// ---------------------------------------------------------------------------

export function createSwitchAnalogElement(
  nodeIds: number[],
  _branchIdx: number,
  props: PropertyBag,
): AnalogElement {
  ...
  const closed = props.has("closed")
    ? (props.get("closed") as boolean)
    : false;
  const g = closed ? 1 / RELAY_R_ON : 1 / RELAY_R_OFF;
  ...
  return {
    ...
    stamp(s: SparseSolver): void {
      stampG(s, nodeA, nodeB, g);
    },
  };
}

// ---------------------------------------------------------------------------
// SwitchDT analog factory — DPDT variable resistance
//
// Pin nodeIds order (SPDT, 1 pole):
//   nodeIds[0] = A1 (common)
//   nodeIds[1] = B1 (upper, connects when closed=true)
//   nodeIds[2] = C1 (lower, connects when closed=false)
// ---------------------------------------------------------------------------

export function createSwitchDTAnalogElement(
  nodeIds: number[],
  _branchIdx: number,
  props: PropertyBag,
): AnalogElement {
  ...
  const gAB = closed ? 1 / RELAY_R_ON : 1 / RELAY_R_OFF;
  const gAC = closed ? 1 / RELAY_R_OFF : 1 / RELAY_R_ON;

  return {
    nodeIndices: [nodeA, nodeB, nodeC],
    branchIndex: -1,
    isNonlinear: false,
    isReactive: false,

    stamp(s: SparseSolver): void {
      stampG(s, nodeA, nodeB, gAB);
      stampG(s, nodeA, nodeC, gAC);
    },
  };
}
