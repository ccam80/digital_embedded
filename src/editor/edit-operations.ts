...
/**
 * Duplicate selected elements and wires in place (offset by 1 grid unit on
 * each axis). Equivalent to copy then paste with a fixed offset.
 * Returns a reversible EditCommand.
 */
export function duplicate(
  ...
  const DUPLICATE_OFFSET = { x: 1, y: 1 };
  const newElements: CircuitElement[] = [];
  const newWires: Wire[] = [];
  ...
  return {
    ...
    execute(): void {
      ...
      for (const el of elements) {
        ...
        const props = el.getProperties().clone();
        const cloned = def.factory(props);
        cloned.position = {
          x: el.position.x + DUPLICATE_OFFSET.x,
          y: el.position.y + DUPLICATE_OFFSET.y,
        };
        cloned.rotation = el.rotation;
        cloned.mirror = el.mirror;
        newElements.push(cloned);
        circuit.addElement(cloned);
      }
      ...
      for (const wire of wires) {
        const cloned = new Wire(
          { x: wire.start.x + DUPLICATE_OFFSET.x, y: wire.start.y + DUPLICATE_OFFSET.y },
          { x: wire.end.x + DUPLICATE_OFFSET.x, y: wire.end.y + DUPLICATE_OFFSET.y },
        );
        newWires.push(cloned);
        circuit.addWire(cloned);
      }
    },
    undo(): void {
      for (const el of newElements) {
