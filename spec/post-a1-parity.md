...
| M-W3-4 | mos1load.c:507 (`von = MOS1tVbi * type + MOS1gamma * sarg`) | mosfet.ts:1255 | **SPEC CITATION ERROR (2026-04-24).** Original remedy cited `mos1temp.c:167` as the source of a temperature-corrected `tGamma`. Verified against ngspice source: `mos1temp.c:167` is `tSurfMob = MOS1surfaceMobility/ratio4`, not a gamma correction; no `MOS1tGamma` field exists in ngspice; `mos1load.c:507` reads `model->MOS1gamma` directly, which is the unchanged model-level input. digiTS's `params.GAMMA` is therefore ngspice-correct. | — | **CLOSED — spec-citation error.** No code change; `params.GAMMA` matches `model->MOS1gamma` semantics. W4.B.8 verifier (2026-04-24) + user ruling B.8-a. |
...
**D-8 canary status:** CARRIED FORWARD to Phase 6 as PARITY ticket (user ruling 2026-04-24). Slot `SLOT_CAP_IEQ_DB` that held the failing `-3.5e-12` was excised by A1. Quantity now lives in `niIntegrate` output at mosfet.ts:1340–1388. Harness comparison against ngspice at Phase 6 acceptance resolves the item.
...
**Net remaining carry-forwards:** 1 item (D-8 deferred to Phase 6 as PARITY ticket per user ruling 2026-04-24). 6 items closed.
