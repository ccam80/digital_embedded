...
**digiTS file:** `src/components/switching/fgnfet.ts`
...
FGNFET is a composite (MOS + CAP). It has no `ngspiceNodeMap` of its own.
...
|---|---|
| `MOS1gNode` (gate) | `fgNode` (the floating-gate internal node allocated by `ctx.makeVolt`) |
...
```
This is the digiTS port of `CKTmkVolt` (`cktmkvol.c:20-41`). The MOS sub-element's gate and the CAP sub-element's positive terminal both reference this node.
...
load(ctx: LoadContext): void {
  this._cap.load(ctx);  // stamps CAP conductance/charge onto fgNode/ground
  this._mos.load(ctx);  // stamps MOS transconductance, output conductance, etc.
}
```
