...
**digiTS file:** `src/solver/analog/behavioral-gate.ts` (factory: `makeXorAnalogFactory`)
...
This single spec file covers XOR-2, XOR-3, XOR-4, and any N-input XOR variant. The factory signature is `makeXorAnalogFactory(inputCount: number)`. When `inputCount === 0` the factory reads `props.inputCount` at instantiation time (defaulting to 2). The truth function `xorTruth` counts the number of HIGH inputs and returns `true` when that count is odd- this generalises correctly to N inputs. The setup() for-loop over `this._inputs` covers all input counts without per-N branching.
...
For an N-input XOR gate:

1. `In_1`- first input
2. `In_2`- second input
3. ...
4. `In_N`- Nth input
5. `out`- XOR output (HIGH when an odd number of inputs are HIGH)
...
setup(ctx: SetupContext): void {
  ...
  // Covers XOR-2, XOR-3, XOR-4, ..., XOR-N without per-N branching.
...
Implementer keeps the existing `load()` body verbatim BUT removes any `solver.allocElement` calls. Pin models stamp through the handles cached during their `setup()`. Capacitor children stamp through their own cached handles. The truth-table evaluation (`xorTruth`), latching logic (`_latchedLevels`), and `_output.setLogicLevel(outputBit)` remain unchanged.
...
For an N-input XOR gate: total TSTALLOC count = **N + 1** (N input pins + 1 output pin, when all loaded). Capacitor children add their own 4 entries each (per `PB-CAP.md`).

Examples:
- XOR-2: 3 TSTALLOCs (before capacitors)
- XOR-3: 4 TSTALLOCs
- XOR-4: 5 TSTALLOCs

## Factory cleanup

- Drop `internalNodeIds` and `branchIdx` from the `makeXorAnalogFactory` closure signature (per A6.3). The factory currently ignores them (`_internalNodeIds`, `_branchIdx`); remove the parameters entirely once A6.3 lands.
...
- **`BehavioralGateElement.setup()` ownership**: the shared method's body lands in W2 per W2.7 (see plan.md ss"Wave plan" and 00-engine.md ssA3.2). This W3 task does NOT write the method- it CONFIRMS the method exists and that the class still imports correctly. The factory-cleanup work for this gate type (drop legacy fields per the standard PB-* factory-cleanup contract) remains in scope for this W3 task.

## State pool

`BehavioralGateElement.stateSize` aggregates `_childElements[].stateSize` (capacitor children only; `GATE_COMPOSITE_SCHEMA` is empty). `stateBaseOffset` is set by `MNAEngine._setup()` via `allocateStateBuffers` per `00-engine.md` ssA5.1. `initState(pool)` distributes offsets to children (existing pattern in `BehavioralGateElement.initState` preserved unchanged).
