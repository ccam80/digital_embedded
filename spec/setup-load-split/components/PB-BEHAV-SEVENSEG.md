...
**`createSegmentDiodeElement.setup()` ownership**: the helper's `setup()` body lands in W2 per W2.6 (see plan.md ss"Wave plan" and 00-engine.md ssA3.2). PB-BEHAV-SEVENSEG's W3 task does NOT write the helper- it only confirms the helper has a `setup` property. This eliminates the previously-documented race with PB-BEHAV-BUTTONLED.
