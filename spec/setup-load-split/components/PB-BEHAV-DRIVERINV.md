...
| `selPin` | `DigitalInputPinModel` | 1 (enable input, active-low) |
...
Identical composition to DRIVER. The only difference is the load() logic: `setHighZ(latchedSel)` instead of `setHighZ(!latchedSel)`. setup() body is identical to DRIVER.
...
load(ctx: LoadContext): void {
  ...
  // Active-low: hiZ when sel is HIGH (inverted from DRIVER)
...
Allocated by sub-element setup() calls:
