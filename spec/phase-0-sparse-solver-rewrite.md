...
- **Files to modify**:
  - `src/solver/analog/sparse-solver.ts` — Remove `_cooRows`, `_cooCols`, `_cooVals`, `_cooCount`, `_cooToCsc`, `_prevCooCount`, `_bldColCount`, `_bldColPos`, `_bldBucketRows`, `_bldBucketCooIdx` fields. Remove `_growCOO()`, `_buildCSC()`, `_refillCSC()`, `_buildLinkedMatrix()` methods. Delete the value-addressed `stamp(row, col, value)` method. Add `allocElement(row, col): number` and `stampElement(handle, value): void`. Rewrite `beginAssembly()` to zero A-entry element values and remove fill-in entries via chain walk. Rewrite `finalize()` to compute Markowitz counts from the persistent linked structure (no CSC build).
  - All callers currently invoking `solver.stamp(row, col, value)` — MNAAssembler, cktLoad (Phase 2), every element's `load()` implementation (Phase 6) — migrate to the handle-based API: cache `allocElement` results at compile time, use `stampElement(handle, value)` in the hot path. This is a cross-phase API change coordinated via Appendix B cktLoad pseudocode and the Phase 6 LoadContext contract.
