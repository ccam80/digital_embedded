...
|---|---|---|
| `ngspiceNodeMap` field on `ComponentDefinition` | yes | §Mechanism, `src/core/registry.ts` |
| `ngspiceNodeMap` field on `MnaModel` | yes/partial | §Mechanism; file path is wrong — see D1 |
...
| Verification: `pin-map-coverage.test.ts` added | yes | §Verification describes the test |
| Verification: `setup-stamp-order.test.ts` implicitly verifies maps | yes/partial | §Verification has wrong section cross-ref — see M1 |
...
|----|----------|----------|---------|--------------|
| M1 | minor | §Verification, first sentence: "A1.7's `setup-stamp-order.test.ts` (per `00-engine.md` §A9)" | The leading "A1.7's" is wrong. A1.7 defines `_getInsertionOrder()`. `setup-stamp-order.test.ts` is created in §A9. The parenthetical correctly cites §A9, making the sentence self-contradictory. | Replace "A1.7's `setup-stamp-order.test.ts` (per `00-engine.md` §A9)" → "A9's `setup-stamp-order.test.ts` (per `00-engine.md` §A9)" |
...
- **Location**: §Mechanism, subsection "`src/solver/analog/types.ts` (or wherever `MnaModel` lives) — `MnaModel`"
...
- **Options**:
  ...
  - **Option C — Keep hedge only**: Leave as-is; rely on implementers using Grep when the file is not found at the primary path.
    ...
    - Cons: W3 agents are spec-forbidden from reading existing source per `plan.md` §"Wave-by-wave reading guide". They would have no safe way to locate the file.

---

#### D2 — `"ctrl"` value absent from `pin-map-coverage.test.ts` allowlist (major)

- **Location**: §Verification, requirement 3: the allowlist of valid `ngspiceNodeMap` values; §Switching — composites table, NFET/PFET row.
- **Problem**: The spec defines two NFET/PFET rows in the "Switching — composites" table:
  ...
  The value `"ctrl"` does not appear in the §Verification allowlist: `"pos"`, `"neg"`, `"drain"`, `"gate"`, `"source"`, `"bulk"`, `"col"`, `"base"`, `"emit"`, `"posNode1"`, `"negNode1"`, `"posNode2"`, `"negNode2"`, `"contPos"`, `"contNeg"`.
...
- **Options**:
  - **Option A — Add `"ctrl"` to the allowlist**: Add `"ctrl"` to the §Verification allowlist enumeration.
...
- **Location**: §Controlled sources — primitive, CCCS row; Note on sense pins.
...
#### I1 — `MnaModel` field description states "see `00-engine.md` §A3.1" without an inline interface fragment (info)

- **Location**: §Mechanism, `MnaModel` subsection: "Same field shape — see `00-engine.md` §A3.1."
- **Problem**: This is a forward reference without duplication, which is correct practice. However, `00-engine.md` §A3.1 shows the full `MnaModel` interface including fields that ARE NOT yet present in `src/compile/types.ts` (the current file has `factory`, `getInternalNodeCount`, `getInternalNodeLabels`, `branchCount` — all of which are being replaced/extended by A3.1). The spec correctly treats §A3.1 as the post-migration interface. The reference is accurate as written. No fix needed, flagging for awareness.
