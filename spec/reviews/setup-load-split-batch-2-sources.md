...
- **Location**: PB-VCVS §"setup() body — alloc only"
- **Problem**: The VSRC-family specs (PB-VSRC-DC, PB-VSRC-AC, PB-VSRC-VAR) all use an explicit idempotent guard in setup():
  ...
  ```
  The spec text says `ctx.makeCur` is "idempotent for the same (label, suffix) pair (per 00-engine.md §A2)" to justify this, but 00-engine.md §A2 does NOT state that `makeCur` is idempotent — `makeCur` is defined as calling `engine._makeNode` which always increments `_maxEqNum`. The idempotent guard in VSRC specs is on the *element* (`if (this.branchIndex === -1)`), not on `ctx.makeCur`. The cross-reference to "00-engine.md §A2" as authority for `ctx.makeCur` idempotency is incorrect.
...
- **Options**:
  ...
  - **Option B — Declare `makeCur` idempotent in 00-engine.md and propagate**:
    Amend 00-engine.md §A2 `makeCur` contract to state "returns same number if called with same (deviceLabel, suffix)"; remove element-level guards from all VSRC specs too; leave VCVS as-is.
...
- **Location**: PB-VCVS §"findBranchFor", code snippet
...
- **Location**: PB-CCVS §"findBranchFor", code snippet
...
- **Location**: PB-ANALOG_SWITCH §"load() body — read ctrl, delegate to SW sub-elements"
...
- **Location**: PB-ANALOG_SWITCH §"load() body — SPDT", line `this._swNC.setCtrlVoltage(-vCtrl);`
...
- **Location**: PB-RELAY §"Branch rows" and §"findBranchFor"
...
- **Location**: PB-SUBCKT §"findDevice usage", `_deviceMap` population paragraph
...
- **Why decision-required**: Resolving this requires a choice about how sub-elements are exposed for `findDevice`:
  ...
  - **Option B — Sub-elements register themselves during their own `setup()` call**:
    ...
    - Cons: Changes the `SetupContext` interface (adds `registerDevice`); not in 00-engine.md §A2.
...
- **Problem**: All five specs show `findBranchFor` implementations but use two different internal patterns:
  ...
  **VCVS/CCVS** — uses `ctx.findDevice(name)` which is the correct single-lookup approach consistent with 00-engine.md §A4.2's `_deviceMap`.
...
- **Why decision-required**: Which pattern should all five specs use?
  ...
  - **Option B — All five use a model-instance walk (ngspice pattern)**:
    ...
    - Cons: Factory must collect instances into a closure array; requires 00-engine.md §A4 to specify how the instance list is passed into the MnaModel at compile time.
...
| ID | Severity | Location | Problem | Proposed Fix |
|----|----------|----------|---------|--------------|
| BATCH2-M1 | minor | PB-SW/PB-SW-DT/PB-TRANSGATE §"Verification gate" item 2 | References `switches.test.ts` without confirming TransGate is tested there vs a dedicated file | Verify filesystem; if `switches.test.ts` covers all three, add a note "(covers SW, SW-DT, TransGate)" |
...
- **Applies to**: PB-VCVS
- **Problem**: PB-VCVS §"TSTALLOC sequence" states the anchor is `vcvsset.c:53-58`. The actual ngspice file `ref/ngspice/src/spicelib/devices/vcvs/vcvsset.c` has the 6 TSTALLOC calls at lines 53-58. This is correct. However the spec says:
  > "The idempotent guard (`if (branch == 0)`) is mirrored by `ctx.makeCur` being idempotent for the same (label, suffix) pair (per 00-engine.md §A2)."
  
  This claim is factually wrong — `ctx.makeCur` per 00-engine.md §A2 simply delegates to `engine._makeNode` which always increments `_maxEqNum`. Idempotency in ngspice comes from the `if(here->VCVSbranch == 0)` guard in the element, not from `CKTmkCur`. This is a documentation error (not a code error) that could mislead an implementer who reads this spec before implementing `makeCur`.

| ID | Severity | Location | Problem | Proposed Fix |
|----|----------|----------|---------|--------------|
| BATCH2-M3 | info | PB-VCVS §"Branch rows" | Claims "ctx.makeCur being idempotent for the same (label, suffix) pair (per 00-engine.md §A2)" — 00-engine.md §A2 does not state this; idempotency is via the element-level `if (branchIndex === -1)` guard | Delete the parenthetical "(per 00-engine.md §A2)" and replace with "The idempotent guard must be replicated in setup() as `if (this.branchIndex === -1)` (mirroring vcvsset.c:41-44)." |
...
| ID | Severity | Location | Problem | Proposed Fix |
|----|----------|----------|---------|--------------|
| BATCH2-I1 | info | PB-ISRC §"load() body" | `m` multiplicity parameter referenced without specifying whether it's an existing property or new | Add one sentence: "If `ISRCmValue` is not an existing property on `CurrentSourceElement`, add it with `setParam("m", value)` routing. Default: `1.0`." |

---

#### BATCH2-I2 — SW spec omits model-level default-value processing from `swsetup.c:28-41` (info)

- **Applies to**: PB-SW §"setup() body"
...
| ID | Severity | Location | Problem | Proposed Fix |
|----|----------|----------|---------|--------------|
| BATCH2-I2 | info | PB-SW §"setup() body" | swsetup.c:28-41 model defaults block not mentioned; implementer may wonder whether setup() needs to handle it | Add one sentence explaining model defaults are set at compile time / factory registration |

---

#### BATCH2-I3 — PB-SUBCKT setup() does not address the idempotency gate (`_isSetup`) for the composite (info)

- **Applies to**: PB-SUBCKT §"setup() body"
- **Problem**: `MNAEngine._setup()` has an early-return idempotent guard (`if (this._isSetup) return;` per 00-engine.md §A4.2). If the engine calls `_setup()` a second time (which is valid per the spec), the per-element loop calls `el.setup(ctx)` again for every element including the subcircuit composite. The composite forwards to its sub-elements, meaning every sub-element's `setup()` is called a second time. Some sub-elements (VSRC, IND, VCVS, CCVS) have their own element-level idempotent guard (`if (this.branchIndex === -1)`), so they are safe. But the subcircuit composite itself does not specify whether it should have its own guard or rely on the engine-level `_isSetup` flag to prevent double invocation.
...
- **Proposed fix**: A cross-reference note: "The engine's `_isSetup` guard (00-engine.md §A4.2) prevents `setup()` from being called twice; no composite-level guard is needed."

| ID | Severity | Location | Problem | Proposed Fix |
|----|----------|----------|---------|--------------|
| BATCH2-I3 | info | PB-SUBCKT §"setup() body" | Does not explain double-invocation safety; implementer may add redundant guard | Add cross-reference to 00-engine.md §A4.2 `_isSetup` guard |
...
2. **PB-SUBCKT** (critical ×1): `_deviceMap` is populated from `compiled.elements` (top-level only) per 00-engine.md §A4.1, so `ctx.findDevice` will return `null` for any sub-element inside a subcircuit. Any subcircuit containing a CCCS/CCVS/MUT that references another internal element will fail at setup time.
