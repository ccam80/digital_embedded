...
| PB-BEHAV-SEVENSEGHEX open blocker reflected | partial | §Shape rule 7 table says "same as SevenSeg" without addressing the 2-pin vs 8-pin mismatch noted in plan.md |
| Per-task W3 verification gate for behaviorals | yes | §Per-task verification gate present |
| ngspice-anchor-status justification | yes | §ngspice-anchor status: NONE present |
| No setup-stamp-order row for behaviorals | yes | Explicitly stated |
| Pin-map field: behaviorals do not populate ngspiceNodeMap | yes | §Pin-map field section present |
...
|----|----------|----------|---------|--------------|
| M1 | minor | §Shape rule 1 code block, line `if (this._inputCap)` | Field name `_inputCap` does not exist on `DigitalInputPinModel`. The actual field is `_capacitorChild` (private). The spec code block accesses a private field by name that differs from the source. | Replace `this._inputCap` with `this._capacitorChild` throughout the Shape rule 1 code block. Same rename for the forwarding call: `this._inputCap.setup(ctx)` → `this._capacitorChild.setup(ctx)`. |
...
- **Location**: `02-behavioral.md` §Shape rule 2, code block for `DigitalOutputPinModel.setup(ctx)`
...
- **Location**: `02-behavioral.md` §Shape rule 3, code block for composite `setup(ctx)`
...
- **Location**: `02-behavioral.md` §Shape rule 2, `role === "branch"` guard
...
- **Location**: `02-behavioral.md` §Shape rule 7 and the component-list table row for `SevenSegHex`
- **Problem**: The plan (`plan.md` §Open blockers) states:
...
- **Location**: `02-behavioral.md` §Shape rule 9, the presented `setup(ctx)` code block
...
- **Location**: `02-behavioral.md` §Per-task verification gate (W3), item 1
- **Problem**: The spec reads:
...
- **Location**: `02-behavioral.md` §Shape rule 1, first two lines of `setup()` body
- **Problem**: The spec reads:
  ```ts
  setup(ctx: SetupContext): void {
...
- **Location**: `02-behavioral.md` §Shape rule 3, component list and §Shape rule 4
