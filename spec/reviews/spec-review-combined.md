...
- ngspice findings (D1, D2, D4) and general findings (D2 internal-node IDs, D6 PropertyBag scope) do not overlap; both lanes touch ssA.16/A.17 from different angles.
- F-code D5 (compiler.ts missing F1/F14/F15) and F-code D7 (compiler.ts missing F18) are best fixed together with general's M2 wording fix in ssA.21.
...
General's grep confirmed: no occurrences of "tolerance", "equivalent", "intentional divergence", "close enough", "mapping table", or "pre-existing" used as closing verdicts. Single hit on `partial` is a process-status enum value in ssC.4, not a parity verdict.
...
|----|----------|----------|---------|--------------|
| FC-M1 | major | ssB.5 row `inductor.ts` | F7 listed but file has zero `ctx.makeVolt(` sites | Strike F7. Row becomes `F1, F4, F6` |
| FC-M2 | major | ssB.5 row `tapped-transformer.ts` | F7 listed but file has zero `ctx.makeVolt(` sites | Strike F7. Row becomes `F1, F2, F3, F4, F6` |
| FC-M3 | minor | ssB.7 row `relay-dt.ts` | F1 listed but no `isReactive`/`isNonlinear` matches | Drop F1. Row becomes `F7 (1 makeVolt site for "coilMid")` |
| FC-M4 | minor | ssB.6 rows `varactor.ts`, `schottky.ts`, `diac.ts` | F1 may not be needed per audit | Verify each per-file; drop F1 where grep returns zero hits |
| GEN-M1 | minor | ssA.16 ~line 604 | `SetupContext.solver` doesn't disambiguate `beginAssembly`/`endAssembly` lifecycle | Add footnote: "Tests construct the solver and call `solver.beginAssembly(...)` / `solver.endAssembly()` outside the SetupContext lifecycle; element setup() calls only `ctx.solver.allocElement(...)`." |
| GEN-M2 | minor | ssA.21 ~line 810 vs ssA.15 ~line 480 | Wording about `stateSize` reads as a prohibition | Reword to "The composite's `stateSize` is provided by the `CompositeElement` base-class getter (A.15), which dynamically sums sub-element `stateSize`. The compiler does not write a `stateSize` field to the composite directly." |
| GEN-M3 | major | ssC.1 row C13 ~line 1226 | Pattern `\bel\.isReactive\b` uses literal `el\.` anchor- misses `element.isReactive`, `c.isReactive`, etc. | Replace with `\b\w+\.(isReactive\|isNonlinear)\b`, OR drop C13 entirely (C1+C2 already catch all surface forms) |
| GEN-M4 | minor | ssA.5 / ssA.16 | `allocStates`/`makeCur` JSDoc carries storage notes that belong in prose | Move "Stored to `el._stateBase`"/"Stored to `el.branchIndex`" sentences out of A.16 JSDoc into A.5 prose |
...
- B) Keep two-step composition; rewrite ssA.16 JSDoc to characterize as digiTS-specific; file in `architectural-alignment.md`
...
ssA.17 lists `iabstol` alongside `reltol`. ngspice has `CKTabstol` (cktdefs.h:199), no `CKTiabstol`.
- A) Rename to `abstol`
...
- A) Strengthen F18 description in ssB's edit-codes table
...
Grep shows 4 live flag decls in the file. Row ssB.1 lists `F2, F11, F18` but not F1.
...
`capture.ts:203` reads `el.stateBaseOffset`. Row ssB.11 doesn't list F9.
- A) Add F9
...
ssD.3 accepts no tsc-clean / no test-clean during wave; convergence pass has no exit condition.
