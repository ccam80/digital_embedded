...
| W3 verification: no setup-stamp-order.test.ts row for behaviorals | yes | Stated in 02-behavioral.md ssPer-task verification gate; confirmed in each spec |
...
- **Location**: PB-BEHAV-AND through PB-BEHAV-NOT ssVerification gate, item 1
...
- **Location**: PB-BEHAV-MUX ssState pool
...
- **Options**:
  - **Option A- Add source line citation**: Add: "Constant name confirmed at `src/solver/analog/behavioral-combinational.ts`- implementer reads this line before proceeding."
    ...
    - Cons: W3 agents are forbidden from reading existing source; contradicts plan.md ssWave-by-wave reading guide.
...
- **Location**: PB-BEHAV-SPLITTER ssload() body- value writes only
...
- **Location**: PB-BEHAV-BUTTONLED ssDependency on SEVENSEG migration
...
- **Location**: PB-BEHAV-BUTTONLED sssetup() body and ssPin model TSTALLOCs
...
- **Location**: PB-BEHAV-SEVENSEG ssSegmentDiodeElement setup() body (Shape rule 7), final note
...
- **Location**: PB-BEHAV-SEVENSEGHEX ssPin layout and ssSegmentDiodeElement setup() body (implementer note)
...
- **Location**: PB-BEHAV-GROUND ssNotes on no-op status, last sentence
...
- **Location**: All gate and behavioral specs ssVerification gate, item 4
...
| ID | Severity | Location | Problem | Proposed Fix |
|----|----------|----------|---------|--------------|
| BATCH5-M1 | minor | PB-BEHAV-AND, NAND, OR, NOR, XOR, XNOR, NOT ssVerification gate item 1 | Each spec says "`src/solver/analog/__tests__/behavioral-gate.test.ts` (or equivalent test file for gates)". The file exists (confirmed). The "or equivalent" hedge creates ambiguity. | Remove "(or equivalent test file for gates)" from all seven specs. The confirmed path is sufficient. |
