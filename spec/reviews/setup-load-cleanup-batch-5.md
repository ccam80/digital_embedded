...
### G-02- spec ssB.13 mandate: `or/nor/xor` factory tests in behavioral-gate.test.ts are trivial existence checks only

**Spec requirement (ssB.13):** After deleting flag-only `it()` blocks, the replacement tests for factory functions must verify element behaviour, not just field existence. The spec says "Delete `it()` blocks dedicated solely to flag assertions"- the replacement should assert meaningful behaviour.  
