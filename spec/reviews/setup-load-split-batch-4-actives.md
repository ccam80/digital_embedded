...
|----|----------|----------|---------|--------------|
| FDIAC-M1 | minor | PB-DIAC §"Sub-element 1: DIO D_fwd" ngspiceNodeMap | Map reads `{ A: "pos", K: "neg" }` with key `K` but DIAC has no pin labelled `K` — the parent pin is `B` (cathode of D_fwd). The description text says "A=anode maps to pos, B=cathode maps to neg" which is correct, but the map key is wrong. | Replace `{ A: "pos", K: "neg" }` with `{ A: "pos", B: "neg" }` for D_fwd; similarly for D_rev replace `{ A: "pos", K: "neg" }` with `{ B: "pos", A: "neg" }` (posNode=B, negNode=A for the antiparallel diode). |

#### Decision-Required Items

##### FDIAC-D1 — posPrimeNode identity when RS=0 is not specified (major)

- **Location**: PB-DIAC §"Internal nodes"
...
- **Options**:
  ...
  - **Option B — Cross-reference PB-DIO**: Add: "RS=0 alias rule: identical to PB-DIO §'Internal nodes' — implementer must read PB-DIO before implementing D_fwd/D_rev setup bodies."
...
- **Location**: PB-SCR §"setup() body" code block
...
- **Options**:
  ...
  - **Option B — Declare setPinNode on AnalogElementCore**: Add `setPinNode(pinLabel: string, nodeId: number): void` to the `AnalogElementCore` interface in `00-engine.md` §A3 and implement it on all element classes.
...
- **Location**: PB-SCR §"Verification gate"
...
- **Location**: PB-TRIAC §"setup() body" code block
...
- **Location**: PB-TRIAC §"Four-transistor latch"
...
| ID | Severity | Location | Problem | Proposed Fix |
|----|----------|----------|---------|--------------|
| FSPARK-M1 | minor | PB-SPARK §"State slots" | First line reads "2." (a numbered list item starting at 2) — the section has no item "1." This appears to be a formatting artifact where the section header and numbered list collided. | Remove the stray "2." prefix from the state slots section opener. |
...
- **Location**: PB-ADC §"load() body" `accept()` method, line `const s0 = pool.states[0];`
...
- **Location**: PB-ADC §"load() body" `load()` method, line `if (nEoc > 0) this._eocModel.load(ctx);`
...
- **Location**: PB-ADC §"Sub-element decomposition" and §"load() body"
...
- **Location**: PB-DAC §"Factory cleanup"
- **Problem**: Spec states `hasBranchRow: false` on the composite's `MnaModel`. However, per `00-engine.md` §A3.1, `hasBranchRow` is used by `detectVoltageSourceLoops` and `detectInductorLoops` to correctly size worst-case topology. The DAC composite contains a VCVSElement sub-element (`vcvs1`) which **does** create a branch row. If the composite declares `hasBranchRow: false`, the topology validators may under-count branch rows for circuits containing DACs, potentially missing voltage-source loops.
- **Why decision-required**: The spec's own rule says `hasBranchRow` applies to the composite's MnaModel registration. But the wording in `00-engine.md` §A3.1 is "True for models that allocate a branch row in setup()." The composite's `setup()` calls `vcvs1.setup(ctx)` which allocates a branch row. Does the composite count?
- **Options**:
  ...
  - **Option B — hasBranchRow: false on composite (keep as-is)**: The composite itself does not *directly* call `ctx.makeCur` — only its sub-elements do. The topology validator should walk sub-elements to discover branch rows.
    ...
    - Cons: Topology validators in `00-engine.md` §A6.5 that use `model.hasBranchRow` would need to know to recurse into composites, which is not specified.
  - **Option C — Document the convention explicitly**: Add a note to `00-engine.md` §A3.1 (or to the composite specs) stating: "For composites, `hasBranchRow` refers only to direct `ctx.makeCur()` calls in the composite's own `setup()` body, not to sub-element calls. Topology validators that need to count all branch rows must walk the sub-element tree."
...
- **Location**: PB-DAC §"VCVS TSTALLOC sequence"
...
- **Options**:
  - **Option A — Add cross-reference**: Add: "VCVSElement.setup() skips allocElement calls where row or col is 0 — see PB-VCVS §'Internal nodes: ground handling'. The stamp-order assertion for PB-DAC counts only non-zero-row entries."
...
- **Location**: PB-OPTO §"load() body", line `const s0 = pool.states[0];`
...
- **Options**:
  ...
  - **Option B — Cross-reference PB-DIO**: Add: "See PB-DIO §'State slot names' for DIODE_SLOT_ID and DIODE_SLOT_GEQ constants. Access pattern: `ledSub.stateBaseOffset + DIODE_SLOT_GEQ`."
    - Pros: DRY.
...
- **Location**: PB-OPTO §"setup() body" §"Setup order" steps 4–5 and §"load() body"
...
- **Location**: PB-COMPARATOR §"Sub-element decomposition" and §"Construction"
...
- **Location**: PB-COMPARATOR §"load() body" code block
...
- **Location**: PB-COMPARATOR §"Pin mapping" "IMPORTANT" note
...
- **Location**: PB-OPAMP §"Sub-element decomposition"
...
- **Location**: PB-OPAMP §"Factory cleanup"
...
- **Location**: PB-OPAMP §"Construction" factory body, `const rOut = props.getModelParam<number>("rOut") ?? 75;`
...
- **Location**: PB-REAL_OPAMP §"Sub-element decomposition", capComp row
...
- **Options**:
  ...
  - **Option B — Cross-reference PB-CAP**: Add "capComp node-0 handling follows PB-CAP §'Ground node skipping'." and verify the stamp-order count from that spec.
    - Pros: DRY.
...
- **Location**: PB-REAL_OPAMP §"load() body", slew-rate section
...
- **Location**: PB-SCHMITT §"load() body", line `const outputHigh = this._pool.states[0][this._stateOffset] >= 0.5;`
...
- **Location**: PB-TIMER555 §"load() body", line `const s0 = pool.states[0];`
...
- **Location**: PB-TIMER555 §"setup() body", last lines before CAP children
...
- **Location**: PB-TIMER555 §"load() body", line `this._outModel.setOutputVoltage(vOut);`
...
- **Options**:
  - **Option A — hasBranchRow: true on composites with VCVS sub-elements**: Update all five specs. Update `00-engine.md` §A3.1 to clarify "direct or transitive."
...
- **Location**: PB-SCHMITT, PB-COMPARATOR, PB-OPAMP, PB-REAL_OPAMP, PB-TIMER555 §"Verification gate"
