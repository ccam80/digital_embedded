...
| R8 | W2 stub setup() spec location | Add ssA3.2 to 00-engine.md | `imp-engine` (initial round) | landed |
...
| R9-D2 | _insertionOrder reset behavior | Setup-only lifetime, never reset (ngspice-aligned) | `imp-engine-mega` (ssA1.7) | landed |
...
|----|------|------------|-------------|
| Engine D4 | 00-engine.md ssA5.1 | Add `_poolBackedElements: readonly AnalogElement[]` field to CKTCircuitContext, populated at construction from `circuit.elements.filter(isPoolBacked)` | `imp-engine-mega` |
| Engine D5 | 00-engine.md ssA9 | Use `(engine as any)._setup()` cast in test (private bypass, contained scope) | `imp-engine-mega` |
...
| FREALOP-D1 | (engine + PB-OPAMP, PB-REAL_OPAMP) | Enforce NGSPICE_LOAD_ORDER for sub-elements within composites (engine ssA6.4) | `imp-engine-mega` + `imp-actives` |
...
All ~30 mechanical fixes from the ssMechanical Fixes table in `spec-review-combined.md` were applied in the first round of implementers (`imp-engine`, `imp-plan`, `imp-pinmap-behav`, `imp-batch1` through `imp-batch5`). Spot-verified via per-implementer line-range summaries.
...
| P-plan-D2 | Verifier-agent gated; manual-grep wording removed | plan.md, 00-engine.md ssA7 |
| BATCH1-D2 | C- guard only when shunt structurally possible; new 00-engine.md ssA6.6 codifies | 00-engine.md ssA6.6 |
...
| FBEHAV-BUTTONLED-D1 | C- `createSegmentDiodeElement.setup()` extracted to W2.6 (real body in 00-engine.md ssA3.2) | plan.md, 00-engine.md, PB-BEHAV-BUTTONLED.md, PB-BEHAV-SEVENSEG.md |
| FBEHAV-BUTTONLED-D2 | B- contradictory "future variant" example removed; cathode-zero guard grounded in ssA6.6 | PB-BEHAV-BUTTONLED.md |
...
- The W2 wave now has SIX named sub-tasks: W2.1–W2.5 (existing), W2.6 (createSegmentDiodeElement.setup), W2.7 (BehavioralGateElement.setup). Each W2.6/W2.7 paragraph in 00-engine.md ssA3.2 contains the real body, not a stub.
...
5. **Outstanding blockers** before W3 implementation can begin in earnest:
   ...
   - W2 stub spec (added ssA3.2 in this session) is now ready to drive W2 implementer agents.
