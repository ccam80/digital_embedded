...
- **Location**: spec §A.15 "Subclasses required to refactor (16 classes total)" plus rows in §B.3 (`d-async.ts`, `jk-async.ts`, `rs-async.ts`)
- **Problem**: §A.15's table lists `behavioral-flipflop/d-async.ts | 1 class`, `jk-async.ts | 1 class`, `rs-async.ts | 1 class`. §B.3 assigns `F18` to those three files. But grepping the actual class declarations:
...
- **Options**:
  ...
  - **Option B — Exclude them from A.15**: Remove the three `*-async.ts` rows from A.15's subclass mandate table; drop `F18` from §B.3 for these files (keep `F1`).
...
- **Location**: spec §B.3 row `behavioral-remaining.ts`
...
- **Location**: spec §B.3 rows for the three async-flipflop files
...
- **Location**: spec §B.1 row `bridge-adapter.ts` — currently `F2, F11, F18`
...
- **Location**: spec §B.1 row `compiler.ts` — currently described as "Apply A.21 in full"
...
- **Location**: spec §B.1 row `controlled-source-base.ts`
...
#### D7 — `compiler.ts` is in §B.1 but A.15's mandate also names it — F18 not listed (major)

- **Location**: spec §B.1 row `compiler.ts`; §A.15 "Subclasses required to refactor (16 classes total)" includes `compiler.ts` ("the inline composite literal in `compileSubcircuitToMnaModel` becomes an anonymous class extending `CompositeElement`")
- **Problem**: A.15 explicitly states `compiler.ts`'s inline composite becomes a `CompositeElement` subclass. C.2's R6 confirms: "`extends CompositeElement` on every composite class listed in A.15's subclass mandate. The 16 composite-class files in B.3 plus `bridge-adapter.ts` plus `compiler.ts` (anonymous class)." But the §B.1 row for `compiler.ts` does not list `F18`. F18 is the only F-code that ties to A.15. A C.2 R6 grep on `compiler.ts` will fail because the agent didn't apply F18.
...
- **Location**: spec §B.11 row `capture.ts`
...
- **Options**:
  ...
  - **Option C — Document this as out-of-band**: Treat `stateBaseOffset` survival in `capture.ts` as an A.20 out-of-scope item (harness wire format) and explicitly exempt it.
    ...
    - Cons: contradicts A.1's universal ban and needs §A.20 amended.
...
- **Location**: spec §B.5 row `transmission-line.ts` — `F15 (3 in-file el.isReactive predicates)`
...
- **Location**: spec §B.3 row `behavioral-flipflop.ts`
...
|----|----------|----------|-------------|
| I1 | info | spec §B.6 row `triode.ts` | Spec lists `F1, F4, F5, F12 (class)`. The "(class)" parenthetical is on F12 (correct), but F12's table definition already says "Class adopts unified shape" — the parenthetical is redundant. Stylistic only. |
