...
**Lines:** 88, 89, 98
**Rule violated:** §A.1 / C16 — `stateBaseOffset` is forbidden everywhere; renamed to `_stateBase`.
...
**Rule violated:** §A.15 — subclasses of `CompositeElement` must rely on the base-class `load()` forwarder (or call `super.load(ctx)` after class-specific stamps). They must not hand-roll `loadChildren(this._childElements, ctx)` using the old shared helper.
...
**Rule violated:** §A.15 — same issue as V-02. `BehavioralDFlipflopElement extends CompositeElement` but its `load()` override calls `loadChildren(this._childElements, ctx)` from the legacy shared helper rather than `super.load(ctx)`.
...
**Rule violated:** §A.1 / C6 / C7 / C14 — `withNodeIds` helper is deleted; `allNodeIds` and `pinNodeIds` field-decl forms are forbidden.
...
The potentiometer is in `1.A.passives-4` which is marked `complete`, yet the test file retains three Object.assign calls writing forbidden field names onto elements and one direct `withNodeIds` call. These are the exact patterns §A.1 and C6/C7/C14 prohibit.
...
**Rule violated:** §A.1 / C7 — `pinNodeIds` field-decl form is forbidden.
...
**Rule violated:** §A.1 / C14 — `withNodeIds` helper is deleted.
...
**Rule violated:** §A.1 / C6 / C7 — field-decl forms of `pinNodeIds` and `allNodeIds` are forbidden.
...
**Rule violated:** §A.1 / C6 / C7 — field-decl / field-assignment forms of `pinNodeIds` and `allNodeIds` are forbidden.
...
**Spec requirement:** §B.5 — `tapped-transformer.ts` and `transmission-line.ts` must be fully compliant with §A.
...
**Spec requirement:** §A.15 — all 18 composite classes extend `CompositeElement`; the base class `initState()` handles child state allocation. The `initChildState` helper in `shared.ts` is no longer needed.
...
Source file: clean per grep. `findBranchFor` on base class per §A.6. Associated test file `controlled-source-base.test.ts` has violations (V-08).
...
Source file: clean. Canonical §A.13 pattern. `findBranchFor` on element return literal per §A.6. `label: ""`, `_pinNodes: new Map(pinNodes)`, `_stateBase: -1`, `branchIndex: -1` all present.
...
Source file: clean. `findBranchFor` on element return literal per §A.6.

### 1.A.sources-passives-1 — ground.ts
Source file: clean. 3-arg factory per §A.13. No `setup()` stamps (correct — ground stamps nothing).
...
Source file: clean. `findBranchFor` on class per §A.6. Progress report notes the `name !== this.label` guard in `findBranchFor` differs from the canonical pattern that ignores name — this is a minor deviation from §A.6 but does not introduce a forbidden pattern. The spec's canonical body shows `_name` (unused), the implementation uses `name` with a guard. Flagged for user review rather than reported as a violation because it is conservative and functionally safe.

### 1.A.passives-2 — polarized-cap.ts
Source file: clean. `implements PoolBackedAnalogElement`. `getInternalNodeLabels()` per §A.7. `_internalLabels` reset in `setup()`.
...
Source file: clean. `findBranchFor` on class per §A.6. `props.get<string>("label")` per §A.18. `_pinNodes` re-assigned from `pinNodes` after construction — correct.
...
Source file: clean. `findBranchFor` on element class per §A.6. `getInternalNodeLabels()` per §A.7.
...
Source file: clean. `findBranchFor` on `InductorSubElement` per §A.6. `_pinNodes = new Map([["pos", posNode], ["neg", negNode]])` in constructor.
