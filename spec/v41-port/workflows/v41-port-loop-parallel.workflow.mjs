...
Produce the ORDERED plan (engine tier -1 first, then device tiers 0..3 per gate-manifest.md). Per unit: name, kind, tier; deferred=true+deferReason for: parser CARD-READER (inp2X.c) and MODEL (inpdomod/inpgmod/inpmkmod) groups — the frozen front-end (except parser#recon/nodeAllocOrder which is APPLIED), any unit with an unmapped PENDING hunk, any unit with a specExists:false recon, and zero-PENDING-work units. EMIT, all NO LONGER deferred (gate-manifest.md, 2026-06-03): (a) the from-scratch device classes jfet2 / mes / mos3 / asrc — each has a specExists:true wholeClass recon + a RUNNABLE harness gate whose .dts the GateMerge stage authors against the freshly-built class (asrc#recon/wholeClass builds bsource.ts BV/BI + the netlist-generator B-source emitter and CONSUMES the existing expression engine per asrc-wholeClass.md:686-690 — it does NOT block on expr-engine; its GateMerge fixture uses ALGEBRAIC expressions only: V=2*V(in), I=V(a)*V(b)+I(Vsense), tc1/tc2) (mos3's hunks are mapped in planning/mos3-decisions.json — if you still find an unmapped PENDING mos3 hunk, that classification has not landed, so defer mos3 and say so); (b) the now-placeable csw (RUNNABLE harness gate, csw-gate.dts); (c) the EXPRESSION-ENGINE groups — the parser hunks in inpptree.c / inpeval.c / ptfuncs.c / ifeval.c (mapped to expression.ts / expression-evaluate.ts / model-parser.ts) — emit them as a unit named "expr-engine", gateKind=self-compare per gate-manifest.md. KEEP the parser card-reader + model groups deferred.
