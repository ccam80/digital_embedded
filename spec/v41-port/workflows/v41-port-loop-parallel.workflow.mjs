...
function gateClause(unit, WT, MAIN) {
  ...
  return `call server_restart (the MCP is now pointed at ${WT}), then on EACH fixture [${unit.gateFixtures.join(', ')}]: harness_start (dllPath ${MAIN}/${DLL}) -> harness_run (DC-OP + transient; harness_run_ac for *-ac) -> harness_first_divergence. PASS only if firstDivergence is null across ALL classes on ALL fixtures.`
}
...
Produce the ORDERED plan (engine tier -1 first, then device tiers 0..3 per gate-manifest.md). Per unit: name, kind, tier; deferred=true+deferReason for: parser card-reader/expression/model groups (except parser#recon/nodeAllocOrder which is APPLIED), mos3 (unmapped hunks), any unit with an unmapped PENDING hunk, any unit with a specExists:false recon, manifest-deferred units (csw internalOnly; asrc/jfet2/mes components unbuilt), and zero-PENDING-work units.
