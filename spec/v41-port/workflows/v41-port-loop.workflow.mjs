...
- deferred=true + deferReason for ALL of:
    * parser — the SPICE-deck CARD-READER groups (INP2x), the expression-engine items (-> asrc/#19), and the model/device-card parser items are frozen engine-phase re-plan (#8/#61) — keep ALL of those DEFERRED (never emit them). The ONE exception: \`parser#recon/nodeAllocOrder\` (specExists:true) is engine work (compiler.ts node-numbering). Emit a parser unit carrying ONLY that recon and ONLY its 4 blocked hunks (inppas2.c#h001-h003, inpsymt.c#h001 — tsFunction -> src/solver/analog/compiler.ts). Emit NO other parser groups; the parser unit is "active for nodeAllocOrder only", NOT deferred.
    * mos3 — 43 PENDING hunks are UNMAPPED (tsFunction null); the mapping was never authored. Defer and set deferReason "unmapped hunks — escalate missing mos3->mosfet.ts mapping".
    * any other unit where >0 of its PENDING hunks are unmapped — defer it and name the count in deferReason (planning gap to escalate).
    ...
    * manifest-deferred units (csw internalOnly; asrc/jfet2/mes components unbuilt; nodeset-ic/tf no input surface).
