...
### ESC-025 — `analysis#recon/opTran` RE-OPENED / BUILDABLE — fixture found (inductor-singularity); remaining = ordinary optran plumbing + driver

> **UPDATE 2026-06-02:** trigger (a) below (the "no `.dts` fixture" finding) is **OVERTURNED** — see the Resolution. A simple inductor-induced branch-current singularity is the gate fixture. The original (a)/(b) text is kept as the historical investigation record.
...
- **Resolution:** _RE-OPENED + BUILDABLE 2026-06-02. Trigger (a) is RESOLVED: the gate fixture is an inductor-induced DC branch-current singularity — `v1 1 0 dc 3 / v2 2 0 dc 5 / l1 1 2 1m` (user's intuition, confirmed against the v41 DLL: singular on `l1#branch`, gmin/source-resistant because gmin is a node-to-ground conductance that cannot constrain a branch current, OPtran settles to a unique 3V/5V OP with zero drift over a 1000× opfinaltime sweep; adding series R makes it converge statically — proving the inductor short is the cause). Trivially `.dts`-expressible (2 vsources + 1 inductor) — NO subcircuit-include or VDMOS-thermal tuning needed. Remaining is the ordinary recon scope: trigger (b) the optran option plumbing (ResolvedSimulationParams + a harness `optran` NgspiceJobAnalysis variant + MCP) + the OPtran driver + the `.dts` fixture. Recon stays PENDING, now genuinely buildable; to be built as a focused recon after the serial flow + cktic._
...
## isrc (2026-06-02) — WORKTREE-TEARDOWN gate+merge pass

Teardown of isolated worktree `.wt/isrc` (gate PASS 28/28; clean fast-forward merge
into `v41-port`; mainSrcClean). Three escalation groups passed in from apply/review,
recorded here verbatim. None is a numerical gate-fail / firstDivergence / matrix-cell /
step-iter bug, so all route here rather than to `spec/fix-list-phase-2-audit.md`. Each
group's per-hunk detail lives in the `### ESC-isrc-*` sections above (brought in by the
merge); these bullets are the teardown-pass record.

- **source=isrc/isrc/isrcacct.c::ISRCaccept | verdict=ESCALATE.** h002/h003/h005 are clean (loop accessor-macro renames + PULSE local decls + the VSRC:->ISRC: fft printf string — all allowed C<->TS differences) and the non-compat path of h004 IS Tier-2 bijective with _acceptNgspice PULSE (ac-current-source.ts:697-756: TD/TR/TF/PW/PER order/zero guards, SAMETIME=|a-b|<=1e-7*PW, the v26 PHASE>0 pulse-cap, period-fold, and every CKTsetBreak target match isrcacct.c:62-156). BUT hunk h004 [docLineRange 130-269] contains an un-ported v41 construct: the if(newcompat.xs){phase=PHASE/360; fmod; deltat=phase*PER; while(deltat>0)deltat-=PER; time+=deltat; tshift=TD-deltat;} arm (isrcacct.c:83-93) and the if(!newcompat.xs && time>tmax) guard (isrcacct.c:98-100) have NO digiTS counterpart (VERIFICATION.md sec2 Tier2 automatic fail). digiTS has no newcompat/compat-mode subsystem. Faithful application requires either a compat-mode subsystem (engine-wide, beyond ac-current-source.ts) or a Phase-0 hunk split to frozen NO-COUNTERPART which the verifier may not perform (sec7). The sibling VSRC carved this exact construct out at Phase-0 (vsrcacct.c newcompat NO-COUNTERPART); the ISRC ledger hunk was never split. h004 left ESCALATED; h002/h003/h005 remain PENDING under the group escalation. ESCALATIONS.md ESC-isrc-acct-newcompat appended. Not closed as deferred/blocked/pre-existing (banned).

- **source=isrc/isrc/isrcload.c::ISRCload | verdict=ESCALATE.** h002,h003,h005-h013,h015 are clean: loop renames + m read; PULSE/SINE/EXP/SFFM/AM/PWL value arms are Tier-2 bijective with evaluateNgspiceWaveform (ac-voltage-source.ts:209-345; SINE operand order FREQ*time*2pi+phase pinned, AM phases-twice quirk preserved, EXP TD2 default TD1+step, PWL paren-removal cosmetic); h015 (ISRCcurrent=m*value) matches getPinCurrents m-scaling. TWO blockers prevent group MATCH: (1) hunk h004 [784-853] contains the un-ported newcompat.xs value-path arm (isrcload.c:104-113) + the if(!newcompat.xs && time>tmax) value=V1 guard (isrcload.c:118-120) — identical construct/resolution to isrcacct.c#h004 and the ratified VSRC vsrcload.c#h004 NO-COUNTERPART carve-out; (2) hunk h014 [972-1007] is the TRNOISE value arm (timezero state machine + trnoise_state_get two-sample interpolation + RTS + DC, isrcload.c:310-353) which evaluateNgspiceWaveform THROWS on (ac-voltage-source.ts:347-359) pending the trnoise_state lifecycle subsystem — same cross-subsystem randnumb gap as the ratified VSRC vsrcload.c#h013 (ESCALATED) and isrcpar.c#h003/#h004. h004+h014 ESCALATED; clean hunks remain PENDING under the group escalation. ESCALATIONS.md ESC-isrc-load-newcompat + ESC-isrc-load-trnoise appended.

- **source=isrc/isrc/isrcpar.c::ISRCparam | verdict=ESCALATE.** h002 [1079-1097] is clean: the ISRC_AC vector parse (switch(value->v.numValue) cases 2/1/0 setting acPhase/acMag + ISRCacPGiven/ISRCacMGiven/ISRCacGiven, isrcpar.c:65-83) is realized in digiTS via the acMagnitude/acPhase paramDefs (defaults 1/0, ac-current-source.ts:349-350) and given-flag logic; the hunk's only v26->v41 content is the /* FALLTHROUGH */ compiler-hint comments + switch(param)->switch (param) whitespace — comment/whitespace = allowed differences (sec4), so a zero-delta TS is line-isomorphic (sec2.4). BUT the group is blocked by h003 and h004, which the APPLIER had already set ESCALATED (TRNOISE trnoise_state_free / TRRANDOM tfree ahead of the *_state_init constructor, isrcpar.c:213-217 / 242-246). Reviewed those escalations per VERIFICATION.md sec6a: they are VALID cross-subsystem Trigger-2 escalations (the trnoise_state/trrandom_state lifecycle is unbuilt; maths-misc#recon/randnumb delivered only SeededRng primitives) — identical to the ratified VSRC precedent vsrcload.c#h013 / vsrcacct.c#h008/h009. UPHELD (not bounced). h002 remains PENDING under the group escalation. ESCALATIONS.md ESC-isrc-par-trnoise-free + ESC-isrc-par-trrandom-free already present (applier-appended). committed=null because no new state change by the verifier for this group (the escalations pre-existed and are upheld unchanged).

---

## sw (2026-06-03) — verifier pass, worktree `.wt/sw`

### ESC-sw-load-troubleElt — `CKTtroubleElt = here` blame assignment has no surviving digiTS counterpart

- **source=sw/swload.c::SWload | hunks=[sw/swload.c#h001, sw/swload.c#h002] | verdict=ESCALATE.**
- **Trigger:** cross-group architecture change (§6) — faithful fix provably requires a file outside the `sw/swload.c::SWload` functionGroup.
- **What is clean.** Both hunks are overwhelmingly whitespace reflow (tabs→spaces), redundant-paren removal, the `SWnextModel`/`SWinstances`/`SWnextInstance` loop-accessor macro change (C linked-list plumbing, no TS counterpart — digiTS invokes per element), the `CKTstates[0]+SWstate`→`CKTstate0[SWswitchstate]` state-index rename, the `SWposPosptr`→`SWposPosPtr` casing rename, and comment removal — all allowed C↔TS differences (VERIFICATION.md §4). The one genuine baseline correction the applier made — the MODEINITFLOAT negative-hysteresis hysteresis-region `else` arm now `throw new Error("bad value for previous state in swload")` (analog-switch.ts:199-202) — is Tier-2 bijective with swload.c:90 `internalerror("bad value for previous state in swload")` (the `internalerror` line is byte-identical on the `-` and `+` sides of the diff, sw.md:829/855; the pre-edit TS had wrongly assigned `HYST_OFF` there — a bad-baseline gap that Tier-2 catches and the applier correctly repaired). All four state-decision modes (MODEINITFIX|JCT, MODEINITSMSIG, MODEINITFLOAT, MODEINITTRAN|PRED), the 4-state sentinel arithmetic, the `g_now` selection and the 2×2 conductance stamp (analog-switch.ts:240-249 ↔ swload.c:135-145) are bijective.
- **The un-ported v41 construct.** swload.c:94-97 — when `current_state != old_current_state` in MODEINITFLOAT, ngspice executes BOTH `ckt->CKTnoncon++;` AND `ckt->CKTtroubleElt = (GENinstance *) here;`. digiTS `swLoadHandles` (analog-switch.ts:205-207) does only `ctx.noncon.value++` and never `ctx.troubleElt = <this element>`. The `CKTtroubleElt = here` assignment (swload.c:96) has no digiTS counterpart (VERIFICATION.md §2 Tier-2 automatic fail). This line is whitespace-reflow-only across v26→v41 (sw.md:861 `-` / 862 `+`), so it is a pre-edit baseline divergence, not a hunk-introduced one — exactly the Tier-2 bad-baseline class.
- **digiTS HAS the field.** `ctx.troubleElt: AnalogElement|null` exists (ckt-context.ts:540, cited as the `CKTtroubleElt` mirror) and other devices populate it (newton-raphson.ts:749 `ctx.troubleElt = el` on the `el.checkConvergence` path). So the gap is a missing port, not an architectural absence — but the SW raises noncon **inside `load()`**, where ngspice's SW (unlike the diode) has no separate convTest callback. The SW element implements no `checkConvergence`, so the NR-layer blame path never fires for it.
- **Why the fix crosses the functionGroup.** Even if `swLoadHandles` set `ctx.troubleElt = <element>` after the noncon bump, the very next driver step clobbers it: `makeTroubleTrackingHandler` (ckt-load.ts:54-61) sets `ctx.troubleElt = null` whenever `noncon.value > 0`. ngspice's cktload.c:65-66 clears **only** `CKTtroubleNode` (`if (ckt->CKTnoncon) ckt->CKTtroubleNode = 0;`) — it does NOT clear `CKTtroubleElt`, so the SW's blame survives. Thus a faithful port of swload.c:96 provably requires editing `src/solver/analog/ckt-load.ts` (functionGroup `analysis/cktload.c::...`, ledger `analysis/cktload.c#h001`) to stop nulling `troubleElt` and match cktload.c:65-66. That is outside `sw/swload.c::SWload`.
- **ngspice:** `ref/ngspice/src/spicelib/devices/sw/swload.c:94-97` (blame set); `ref/ngspice/src/spicelib/analysis/cktload.c:65-66` (only `CKTtroubleNode` cleared, `CKTtroubleElt` survives).
- **digiTS:** `src/components/active/analog-switch.ts:205-207` (`swLoadHandles`, only `ctx.noncon.value++`); `src/solver/analog/ckt-load.ts:54-61` (`makeTroubleTrackingHandler` nulls both `troubleNode` AND `troubleElt`).
- **Architecture change required:** (1) `swLoadHandles` / `AnalogSwitchSPSTElement.load()` set `ctx.troubleElt` to the switch instance on the noncon path (in-scope, analog-switch.ts); (2) `ckt-load.ts` `makeTroubleTrackingHandler` align with cktload.c:65-66 — clear only `troubleNode`, not `troubleElt` (out-of-scope, functionGroup `analysis/cktload.c`). Parts (1) and (2) must be sequenced together or the device-set blame is immediately discarded.
- **Files touched by the faithful fix:** `src/components/active/analog-switch.ts`, `src/solver/analog/ckt-load.ts`.
- **Not numerical:** `CKTtroubleElt` is convergence-blame diagnostic state; it does not enter the matrix/RHS/state solve, so this is not a `fix-list-phase-2-audit.md` firstDivergence/matrix-cell bug — it routes here per §6 as a cross-group source-isomorphism gap. Not closed as "pre-existing"/"diagnostic-only"/"close enough" (banned verdicts).
- **Decision needed from user:** authorize the two-file sequenced fix (set blame in SW load + correct ckt-load.ts to mirror cktload.c:65-66), OR split swload.c:96 `CKTtroubleElt = here` to a Phase-0 frozen NO-COUNTERPART (verifier may not perform a hunk split, §7).
- **Resolution:** _pending_

---

## analysis (2026-06-03) — WORKTREE-TEARDOWN gate+merge pass

Teardown of isolated worktree `.wt/analysis` (no isomorphic committed work — merged=false,
gate skipped; rebase onto `v41-port` clean; mainSrcClean). Five escalation groups passed
in from apply/review, recorded here verbatim. None is a numerical gate-fail /
firstDivergence / matrix-cell / step-iter bug, so all route here rather than to
`spec/fix-list-phase-2-audit.md` (each is target-absent / cross-group / architectural).

### ESC-acsetp-acstart-acstop-validation — `analysis/acsetp.c::ACsetParm` (AC_START/AC_STOP frequency-validation)

- **source=analysis/analysis/acsetp.c::ACsetParm(CKTcircuit *ckt, JOB *anal, int which, IFvalue *value) | verdict=ESCALATE.** acsetp.c#h001/#h002. v41 acsetp.c:25-29/35-39 changes AC_START/AC_STOP reject `rValue<=0.0`->`rValue<0.0` and message `Frequency of 0 is invalid`->`Frequency of < 0 is invalid`. Pre-image case (c) absent: ac-analysis.ts has NO ACsetParm counterpart - AcParams.fStart/fStop are taken as-is and consumed by buildFrequencyArray; the .ac-card parameter ingestion/validation lives in the netlist parser layer (file TBD), outside the named tsFile. Reviewed per VERIFICATION.md sec6a: valid target-absent/cross-group escalation, UPHELD (applier-set ESCALATED, left unchanged). ESCALATIONS.md ESC-acsetp-acstart-validation / ESC-acsetp-acstop-validation appended.
- **Resolution:** _pending_

### ESC-cktsetup-modellist-guards / ESC-cktsetup-prevlastnode-save / ESC-cktsetup-prevlastnode-loop — `analysis/cktsetup.c::CKTsetup`

- **source=analysis/analysis/cktsetup.c::CKTsetup(CKTcircuit *ckt) | verdict=ESCALATE.** cktsetup.c#h002,#h003,#h004. h002: `!CKThead`/`!DEVices` E_PANIC guards - compiler.ts has no per-type model-linked-list nor global device-type registry (verified: grep CKThead/DEVices in compiler.ts finds only a context comment). h003/h004: `prev_CKTlastNode=CKTlastNode` save before DEVsetup - no node-ID high-water mark and no setup->unsetup->re-setup re-entry cycle (matrix rebuilt fresh per compile()); the if(error)-reflow, cp_getvar USE_OMP arg, SetAnalyse HAS_PROGREP move are cosmetic/OUT. Pre-image case (c) absent on all three. Reviewed per sec6a: valid target-absent/cross-subsystem, UPHELD. ESCALATIONS.md ESC-cktsetup-modellist-guards / ESC-cktsetup-prevlastnode-save / ESC-cktsetup-prevlastnode-loop appended. No src delta to compiler.ts (correct).
- **Resolution:** _pending_

### ESC-cktsetup-unsetup-nodeptr / ESC-cktsetup-unsetup-lastnode-check — `analysis/cktsetup.c::CKTunsetup`

- **source=analysis/analysis/cktsetup.c::CKTunsetup(CKTcircuit *ckt) | verdict=ESCALATE.** cktsetup.c#h005,#h006. h005: CKTunsetup `node->ptr=NULL` (was =0) on icGiven||nsGiven nodes - digiTS has no node->ptr matrix-element-pointer cache nor a CKTunsetup teardown (nodeset/IC in ctx.nodesets/ctx.ics maps). h006: the prev_CKTlastNode!=CKTlastNode consistency assert + reset - same missing CKTlastNode high-water-mark / setup-unsetup re-entry architecture as h003/h004. Pre-image case (c) absent. Reviewed per sec6a: valid target-absent/cross-subsystem, UPHELD. ESCALATIONS.md ESC-cktsetup-unsetup-nodeptr / ESC-cktsetup-unsetup-lastnode-check appended.
- **Resolution:** _pending_

### ESC-dctran-ltra-sizeincr / ESC-dctran-termination-test — `analysis/dctran.c::DCtran` (mixed group)

- **source=analysis/analysis/dctran.c::DCtran(CKTcircuit *ckt, | verdict=ESCALATE.** Mixed group. APPLIED hunks: h004 (commented-out failed-soln debug dump removal + 'Using transient initial conditions' stdout notice + strstr->strchr in the stdout print loop - all DRIVER-LAYER OUT diagnostics), h005 (the behavioral bcopy(state0,state1)->memcpy(state1,state0) at MODEINITTRAN, dctran.c:322-323 = copy state0 INTO state1; digiTS already does exactly this at analog-engine.ts:2013 `statePool.states[1].set(statePool.states[0])`, applier updated only the citation comment with present-tense mechanism; plus the OUTendPlot teardown OUT), h006-h009 (XSPICE/IPC/CKTdump-output-gate/stat-counter, DRIVER-LAYER OUT) - recorded APPLIED. ESCALATED hunks (UPHELD per sec6a): h003 (CKTsizeIncr=100 + LTRA CKTtimePoints TMALLOC - LTRA out of frozen device-class scope, no counterpart), h010 (transient done-test simplification - digiTS externalised the for(;;) loop + CKTfinalTime termination to coordinator.ts:521-535 target-driven loop, faithful port touches coordinator.ts not the named tsFile, cross-group). Group verdict ESCALATE because two hunks remain validly escalated. ESCALATIONS.md ESC-dctran-ltra-sizeincr / ESC-dctran-termination-test appended.
- **Resolution:** _pending_

### ESC-transetp-h001 — `analysis/transetp.c::TRANsetParm` (TRAN_TSTOP error-string typo fix in absent IFparm dispatcher)

- **source=analysis/analysis/transetp.c::TRANsetParm(CKTcircuit *ckt, JOB *anal, int which, IFvalue *value) | verdict=ESCALATE.** transetp.c#h001 ESCALATED (applier-raised, verifier UPHELD per VERIFICATION.md 6a). v41 delta (transetp.c:26, analysis.md docLineRange 8405-8413) is a single user-facing C error-string typo fix: copy("TST0P is invalid, must be greater than zero.")->"TSTOP..." inside 'case TRAN_TSTOP' of the IFparm TRANsetParm(which,IFvalue*) dispatcher. The whole case TRAN_TSTOP validation construct is ABSENT in digiTS: transient params (TSTOP/TSTEP/TSTART/TMAX/UIC) flow through SimulationParams+resolveSimulationParams+facade validation, an architectural divergence in WHERE/HOW param validation happens; src search excl tests for TSTOP/TST0P/'must be greater than zero'/'is invalid, must'/TRANsetParm/setParm returns ZERO hits. Reviewed as NOT bogus/in-scope: a faithful port of this string-literal fix provably requires fabricating the entire TRANsetParm IFparm dispatcher subsystem, outside this functionGroup -> legitimate target-absent escalation. ESCALATIONS.md entry ESC-transetp-h001 carries the cited ngspice file (ref/ngspice/src/spicelib/analysis/transetp.c:23-29), the digiTS file (src/core/analog-engine-interface.ts), and the required user/planning resolution (confirm NO-COUNTERPART for the C IFparm dispatcher, OR author a TRANsetParm reconstruction spec). Job not complete until user adjudicates.
- **Resolution:** _pending_

---

## maths-sparse (2026-06-03)

### ESC-maths-sparse-nodesetIcRowZero — recon spec PRESENT + RATIFIED but builder produced ZERO edits (empty-diff catch)
...
## expr-engine (parser/inpptree.c, inpeval.c, ptfuncs.c, ifeval.c) — verifier ESCALATE (2026-06-03)

**Raised by:** group-isomorphism verifier (expr-engine unit, parallel phase, no harness gate).
**Worktree:** `C:/local_working_projects/digital_in_browser/.wt/expr-engine` — working tree CLEAN; the only commit beyond merge-base `a4ac37bd` is `ca384593` (inpeval.c#h005 atto suffix, committed in an earlier pass and NOT staged this pass). No new applier delta exists for any of the four groups.

### Root cause (shared across all four groups)
digiTS's expression engine is an **independent recursive-descent parser producing a typed `ExprNode` discriminated-union AST** (`src/solver/analog/expression.ts`), evaluated by `expression-evaluate.ts` (a plain name -> `Math.*` `BUILTIN_FUNCTIONS` map) and differentiated by `expression-differentiate.ts`. It is **not** a structural port of ngspice's yacc/bison-based B-source parse-tree builder. ngspice's `inpptree.c` machinery — `mkb`/`mkcon`/`mkf` ref-counted tree-node ctors, `PT_mkbnode`/`PT_mkfnode`/`PT_mknnode`/`PT_mksnode`, the `ops[]`/`funcs[]` function-pointer tables, `PTcheck` placeholder pass, `PTlex` lexer, `free_tree`/`INPfreeTree`/`printTree` — **has no digiTS counterpart by design**, so VERIFICATION.md Tier-2's required *bijective* construct correspondence cannot be established. The rename-map (`rename-maps/expr-engine.md`, committed in ca384593) states this explicitly. Per VERIFICATION.md §5 ("they differ and I am unsure whether that is acceptable" = ESCALATE) and §6 (cross-group architecture change is the user's call), this is escalated rather than waved through or marked MISMATCH.

### Group 1 — parser/inpptree.c#h001–h033 -> src/solver/analog/expression.ts
- ngspice: `ref/ngspice/src/spicelib/parser/inpptree.c`. Hunks cover: `ops[]` PT_POWER ptr `PTpower`->`PTpowerH` (diff-doc 8729-30); `funcs[]` `ddt` entry (8746); `PTdifferentiate` PT_POWER/PTF_POW/PTF_PWR rewrites using PTF_POW/PTF_PWR/PTF_ABS + `newcompat.hs/.lt` (8770-9075); PTF_EXP deriv `newcompat.ps`/EXPARGMAX/EXPMAX (8889-8908); **PTF_TAN deriv 1/(cos^2)->1+(tan^2)** and **PTF_TANH deriv 1/(cosh^2)->1-(tanh^2)** (8915-8933); `PTcheck` signature +char* tline (9197); static->exported `PT_mk*node` (9245-9349); `prepare_PTF_DDT` (9254-67); `PTlex` B-source instance-param skip (9359-83); `free_tree`/`INPfreeTree` DDT free + null guard (9387-9434).
- digiTS: `expression.ts` (ExprNode AST + recursive-descent parser/tokenizer). No tree-builder, no node pool, no func-ptr tables.
- **Out-of-fileset target:** the `PTdifferentiate` hunks (h008–h015) map per the rename-map to `src/solver/analog/expression-differentiate.ts`, NOT this group's assigned tsFile `expression.ts`. digiTS's `expression-differentiate.ts:157-159` still has the **pre-v41 tan derivative** `div(one(), pow(callNode("cos",[g]), two()))` (1/cos^2) — v41 changes it to 1+tan^2. Porting that is real numerical work but lands in a file outside this unit's scope.

### Group 2 — parser/inpeval.c#h001–h006 -> src/solver/analog/model-parser.ts
- h001–h004: `isdigit`->`isdigit_c` (`inpeval.c` INPevaluate, diff-doc 4108/4117/4126/4135) — C-locale predicate; digiTS uses regex \d in `parseSpiceValue` (`model-parser.ts:119`). Allowed C<->TS difference, empty delta valid.
- h005: atto suffix 'a'/'A'->expo1-18 — **APPLIED** (`SPICE_SUFFIXES` ["A",1e-18] at `model-parser.ts:96`, commit ca384593).
- h006: `INPevaluateRKM_R/_C/_L` (diff-doc 4151-4876, ~725 lines) — RKM "4k7" notation; **no caller in digiTS, no counterpart**. Feature-presence ruling needed.

### Group 3 — parser/ptfuncs.c#h001–h007 -> src/solver/analog/expression-evaluate.ts
- ngspice `ref/ngspice/src/spicelib/parser/ptfuncs.c`: `PTabs`->fabs; `PTpower`/`PTpowerH` `newcompat.lt/.hs`; `PTpwr` PSPICE `PTfudge_factor`; **`PTexp` arg>227.9559242->1e99 (unconditional) + newcompat.ps EXPARGMAX clamp** (diff-doc 9686-91); **`PTlog`/`PTlog10` arg<0->HUGE, arg==0->-1e99 (unconditional)** (9700-13); `PTddt` transient deriv needing `CKTtime`/`MODETRAN` (9727-71).
- digiTS `expression-evaluate.ts:47-66`: `BUILTIN_FUNCTIONS` are bare `Math.exp`/`Math.log`/`Math.log10`/`Math.pow` — **none of the range clamps or compatmode branches present**. The *unconditional* clamps (PTexp 1e99 ceiling, PTlog/PTlog10 arg<=0 handling) are portable numerical behavior, not compatmode infra; the rename-map filing them under "escalated" should be a user ruling, not an in-line close.

### Group 4 — parser/ifeval.c#h001–h002 -> src/solver/analog/expression-evaluate.ts
- h001: `IFeval` null-tree guard (if !myTree -> fprintf + controlled_exit(EXIT_BAD), diff-doc 132-135) — C process-exit; digiTS uses structured throw.
- h002: `PTeval` adds leading \n to a stderr range-error format string (144-145) — debug-output cosmetic.
- digiTS `evaluate(expr, ctx)` has no controlled_exit/fprintf machinery. Cannot independently confirm a bijective MATCH; §5 forbids benefit-of-the-doubt.

### Decision needed from user
1. **Architecture ruling (Groups 1,3,4):** is the recursive-descent ExprNode engine an accepted architectural divergence from ngspice's yacc parse-tree builder, OR must the portable *numerical* deltas be ported into the digiTS engine — specifically: (a) PTexp 227.9559242->1e99 ceiling + PTlog/PTlog10 arg<=0 clamps into `BUILTIN_FUNCTIONS`/`evaluate`; (b) the PTF_TAN/PTF_TANH derivative corrections into `expression-differentiate.ts` (out-of-fileset — needs sequencing); (c) compatmode (`newcompat.lt/.hs/.ps`) power/exp/pwr branches + `PTddt` transient derivative + `controlled_exit` error machinery, which require infra digiTS lacks.
2. **Feature ruling (Group 2 h006):** is RKM "4k7" notation (`INPevaluateRKM_*`) in scope? No digiTS caller exists.
3. **Fileset (Group 1):** the PTdifferentiate hunks target `expression-differentiate.ts`, outside this unit's assigned tsFile `expression.ts`; needs cross-group sequencing.

**Resolution:** _pending_

## bjt model port (2026-06-03) — VERIFIER ESCALATION: v41 Kull Quasi-Saturation model adds a new internal circuit node (cross-group)

Worktree `.wt/bjt` was verified for the 10 BJT model functionGroups (bjt.c::BJTmPTable, bjtacld.c::BJTacLoad, bjtconv.c::BJTconvTest, bjtdefs.h structs/macros, bjtload.c body + next1 + next2 + load). The worktree `git diff` is EMPTY for every one of the 64 hunks beyond the already-committed `bjt#recon/stampAc` (d85ac696) — none of these v26->v41 model hunks were applied this session. Per VERIFICATION.md §1 zero-delta rule I Tier-2'd the existing bjt.ts against fresh v41 `ref/ngspice/src/spicelib/devices/bjt/`: the existing code is the **v26 baseline** (area-scaling done in load: `csat=tSatCur*AREA`, `gcpr=tcollectorConduct*AREA`, `oik=tinvRollOffF/AREA`, `c2`/`c4`, `rbpr`, `xjrb`; the VERTICAL/LATERAL `c4`/`ctot`/`czsub` branches; the two-junction BJTconvTest). It has ZERO of the v41 additions (no `collCX` / `intCollResist` / `epiSatVoltage` / `epiDoping` / `irci` / `vbcx` / `quasimod` / `IBE` / `IBC` identifiers — confirmed by Grep, 0 matches).

### ESC-BJT-QS-collCXnode — v41 adds a new internal node BJTcollCXNode + 5 matrix cells; blast radius is compiler.ts + setup TSTALLOC, beyond the bjt.ts functionGroup

- **Trigger:** VERIFICATION.md §6 cross-group architecture change (blast radius beyond the functionGroup) + device-class-scope.md binary rule (BJT is IN -> the QS sub-feature is a required reconstruction item, NOT an accepted divergence).
- **ngspice (v41):**
  - `ref/ngspice/src/spicelib/devices/bjt/bjtdefs.h`: new instance field `int BJTcollCXNode` (diff-doc bjt.md:470); 5 new matrix-pointer fields `BJTcollCXcollCXPtr` / `BJTcollCXBasePrimePtr` / `BJTbasePrimeCollCXPtr` / `BJTcolPrimeCollCXPtr` / `BJTcollCXColPrimePtr` (bjt.md:521-530); renamed `BJTcolColPrimePtr`->`BJTcollCollCXPtr`, `BJTcolPrimeColPtr`->`BJTcollCXCollPtr` (bjt.md:502-513); 9 new state slots vbcx/vrci/irci/irci_Vrci/irci_Vbci/irci_Vbcx/qbcx/cqbcx/gbcx pushing BJTnumStates 24->33 (bjt.md:588-620).
  - `bjtsetup.c` (v41) TSTALLOCs the collCX cells and allocates BJTcollCXNode when `BJTintCollResistGiven`.
  - `bjtload.c` Kull QS block (diff-doc bjt.md:2033-2090) + QS stamp block writing `CKTrhs[BJTcollCXNode]` / `CKTrhs[BJTcolPrimeNode]` and the collCX matrix cells (bjt.md:2338-2364).
  - `bjtacld.c` QS AC stamps gated on `BJTintCollResistGiven` (bjt.md:266-283).
  - `bjtconv.c` third-junction `vbcx`/`delvbcx` convergence read from `CKTrhsOld[BJTcollCXNode]` (bjt.md:397-402).
- **digiTS:** `src/components/semiconductors/bjt.ts` BJT L0/L1 element classes allocate a fixed node/TSTALLOC set in `setup()` (no collCX node); the state schemas have no vbcx/vrci/irci slots; `load()` / `stampAc()` / `checkConvergence()` have no QS branch.
- **Why architectural, not in-group:** adding `BJTcollCXNode` is a new MNA unknown. Its allocation ORDER (relative to the existing colPrime/basePrime internal nodes) is owned by the compiler's composite/internal node-ID allocation in `expandCompositeInstance` (compiler.ts) and the device `setup()` TSTALLOC sequence — per CLAUDE.md "Sparse Solver is Settled", node-index allocation order and the TSTALLOC sequence are exactly the upstream surfaces that determine matrix shape and bit-exact parity. This cannot be done inside bjt.ts alone without a ruling on (a) whether digiTS models the QS internal node as a real extra solver node and (b) where in the allocation order it lands.
- **Files the faithful fix touches:** `src/components/semiconductors/bjt.ts` (schema, setup TSTALLOC, load/stampAc/convergence QS branches, param defs RCO/VO/GAMMA/QCO), `src/solver/analog/compiler.ts` (internal-node allocation for the QS collCX node), and the temperature helper `computeBjtTempParams` (tintCollResist/tepiSatVoltage/tepiDoping — bjttemp.c, out of this unit's tsFile).
- **Affected groups (QS-coupled -> ESCALATE):** bjtacld.c::BJTacLoad (bjtacld.c#h001-h003), bjtconv.c::BJTconvTest (bjtconv.c#h001-h003), bjtdefs.h::sBJTinstance (bjtdefs.h#h002-h007), bjtdefs.h::BJTstate-macros (bjtdefs.h#h008), bjtdefs.h::sBJTmodel (bjtdefs.h#h010-h014), bjtload.c body/next1/next2/load (bjtload.c#h001-h038). The model param table bjt.c#h002-h009 (BJTmPTable) is tracked as in-file MISMATCH (param-table additions land in bjt.ts; QS-param *consumption* depends on this escalation).
- **Decision needed from user:** authorize a `bjt#recon/quasiSaturation` reconstruction item with cross-group sequencing (bjt.ts + compiler.ts + bjttemp helper), OR rule the QS sub-feature out (which requires an explicit amendment to device-class-scope.md, a user-only action). The IBE/IBC separate-saturation-current split, the `subSatCurGiven` substrate conditional, and the instance-relative area-scaling refactor (`here->BJTBEtSatCur` etc.) are in-file MISMATCH the applier ports in bjt.ts, separate from this node-allocation escalation.

**Resolution:** _pending_

#### Verifier addendum (2026-06-03, parallel group pass `.wt/bjt`) — `BJTsetup` + `BJTtemp` functionGroups map onto this escalation

This parallel verification of three BJT functionGroups (`bjtsetup.c::BJTsetup`, `bjttemp.c::BJTtemp`, `bjttrunc.c::BJTtrunc` against `src/components/semiconductors/bjt.ts`) confirms two of them are blocked by THIS escalation; `bjttrunc.c::BJTtrunc` (h001) is source-isomorphic and is recorded `APPLIED` (its committed QBCX branch is gated on `_intCollResistGiven`, bijective to bjttrunc.c:33-35).

- **`bjt/bjtsetup.c::BJTsetup` (h001-h008) → ESCALATE.** Commit `e3ab7e01` landed the QS-coupled subset isomorphically (h006 collCX node-resolution `bjtsetup.c:424-446`; h007 TSTALLOC rename `BJTcollCollCXPtr`/`BJTcollCXCollPtr` + unconditional `BJTcollCXcollCXPtr` and the 4 `intCollResist`-gated coupling ptrs `bjtsetup.c:498-539`; the ISE/ISC `>1e-4` spice-2G rescale from h005 `bjtsetup.c:389-403`). But the group is NOT complete: h002 `BEsatCur`/`BCsatCur` `=0.0` defaults (`bjtsetup.c:50-58`), h003 `intCollResist`/`epiSatVoltage`/`epiDoping`/`epiCharge` defaults (`bjtsetup.c:160-178`), h004 `tiss1`/`tiss2`/`quasimod`/`energyGapQS`/`tempExpRCI`/`tempExpVO` defaults (`bjtsetup.c:355-384`), and h005 `icMax`/`ibMax`/`pdMax`/`teMax` SOA defaults (`bjtsetup.c:393-403`) are ABSENT from bjt.ts (Grep: 0 matches for any of these identifiers). These are real v41 constructs (model-param defaults), not §4 zero-delta-allowed cases — they initialize the QS/VBIC/SOA/TLEV subsystem this escalation governs, so the group cannot be `MATCH` until that subsystem is authorized.
- **`bjt/bjttemp.c::BJTtemp` (h001-h011) → ESCALATE.** digiTS's `computeBjtTempParams` (bjt.ts:384-488) is the v26-baseline single-path Gummel-Poon temperature computation: no `BJTtlev`/`BJTtlevc` branch, no per-parameter tempco arrays (`(1+Ttf1*dt+Ttf2*dt*dt)` etc.), no separate `BEtSatCur`/`BCtSatCur` (uses `tBEleakCur = ISE>0 ? ... : 0` directly), no quasimod/epi temperature block (`tintCollResist`/`tepiSatVoltage`/`tepiDoping`, bjttemp.c:215-229), no `mje`/`mjc`/`mjs`>0.999 limiting, no `areab`/`areac` substrate area-scaling. v41 `BJTtemp` (bjttemp.c:19-338) is a structurally larger function whose TLEV/TLEVC + tempco-array + dual-satcur + QS-temperature additions have no digiTS counterpart. Porting it bijectively requires the full QS/VBIC/TLEV subsystem (new model params + state + the `BEsatCur`/`BCsatCur` split this escalation already lists), beyond the `bjttemp.c` functionGroup. Same root cause and same user decision as above.

**Resolution:** _pending_ (subsumed by the parent `bjt#recon/quasiSaturation` decision)

### ESC-027 — `csw/cswtrunc.c::CSWtrunc` (h001 preamble, h002 body) — per-device timestep limiter has no digiTS counterpart; reads an unpopulated `CSWctrlvalue` state slot

- **Trigger:** VERIFICATION.md §2 Tier-2 (a whole v41 function with no digiTS construct counterpart) + §6 cross-group architecture change (the faithful fix touches `CSW_SCHEMA` and `load()`, beyond the `cswtrunc.c` functionGroup). This is the planner's unratified Open Question 2 (`spec/v41-port/planning/csw-review.md:154-172`).
- **affected groups:** `csw/cswtrunc.c::Copyright 1990 Regents of the University of California.  All rights reserved.` (cswtrunc.c#h001), `csw/cswtrunc.c::Modified: 2000 AlansFixes` (cswtrunc.c#h002).
- **ngspice (v41):** `ref/ngspice/src/spicelib/devices/csw/cswtrunc.c:15-52` (`CSWtrunc`). It computes a per-instance `maxStep` from `lastChange = CKTstate0[CSWctrlvalue] - CKTstate1[CSWctrlvalue]` and the switch state `CKTstate0[CSWswitchstate]`, clamping `*timeStep`. The v41→v26 delta in h002 is more than cosmetic: it renames the state addressing from `*(ckt->CKTstate0+(here->CSWstate+1))` / `*(ckt->CKTstate0+(here->CSWstate))` to the named fields `here->CSWctrlvalue` / `here->CSWswitchstate` (structural, not an allowed identifier-rename of an existing TS construct, because no TS construct exists).
- **Critical v41 fact:** the input slot is dead in v41 itself. `ref/ngspice/src/spicelib/devices/csw/cswload.c:132-134` has the store `ckt->CKTstate0[here->CSWctrlvalue] = i_ctrl;` **commented out** behind a FIXME ("without this statement truncation is not used, as with SW"). So `CKTstate0[CSWctrlvalue]` is never written → `lastChange == 0` → the `lastChange > 0` / `lastChange < 0` guards never fire → `CSWtrunc` never reduces the step. The per-device truncation is dormant in v41.
- **digiTS:** `src/components/switching/current-controlled-switch.ts` — `CurrentControlledSwitchAnalogElement` has **no** truncation method (`getLteTimestep` absent; the reactivity discriminant in `element.ts:106-116`), and `CSW_SCHEMA` declares only `CLOSED` + `CSW_STATE` slots — **no `ctrlvalue` slot**, and `load()` never stores `i_ctrl` into state (matching the commented-out v41 store). digiTS uses the generic CKTterr LTE path (`ckt-terr.ts` / `timestep.ts`), the same surface where the engine-level `CKTtrunc` was already escalated (ESC-022).
- **Why architectural, not in-group:** a faithful port would (a) add a new `CSWctrlvalue` slot to `CSW_SCHEMA`, (b) add the `i_ctrl` store into `load()` (de-commenting the v41 FIXME line — a deliberate behavioral change ngspice declined to make), and (c) add a `getLteTimestep` truncation method to the element. (a)+(b) edit the load functionGroup and the state schema, outside the `cswtrunc.c` functionGroup, and de-commenting a FIXME the upstream author left dormant is a behavioral decision, not a mechanical port.
- **Files the faithful fix touches:** `src/components/switching/current-controlled-switch.ts` (`CSW_SCHEMA` slot add, `load()` ctrlvalue store, new truncation method).
- **Decision needed from user:** (A) leave `CSWtrunc` as NO-COUNTERPART — the v41 truncation is dormant (FIXME-disabled), so omitting it is bit-identical to v41 timestep behavior; OR (B) authorize an in-scope gap fill: add the `ctrlvalue` slot + `load()` store + truncation method, de-committing the v41 FIXME (a divergence FROM v41, requiring its own justification). Per CLAUDE.md the dormancy cannot be used to close the item silently as "equivalent"; it is the user's ruling.
- **Resolution:** _pending_

## sw (2026-06-03)

GATE+MERGE+TEARDOWN for unit `sw` did NOT merge. The rebase of worktree `.wt/sw` onto `v41-port` could not start: `git rebase` aborted with "cannot rebase: You have unstaged changes" — the worktree carries one uncommitted modified file, `src/components/active/analog-switch.ts`, holding all three escalated recon edits (acLoad/trunc/icParam) plus the out-of-scope MODEINITFLOAT hunk described below. rebaseClean=false (rebase never started; no content conflict — the block is the uncommitted shared-file work that was correctly NOT committed per the per-recon escalations). Kept work preserved to `C:/local_working_projects/digital_in_browser/.wt-failed/sw.diff` (worktree diff vs v41-port, src/ only, 15879 bytes) for resume-from-patch. MAIN src/ is clean (no leak). The three recon bodies are APPLIED-quality and bit-isomorphic to their swacload.c / swtrunc.c / swparam.c+sw.c+swload.c+swdefs.h references (per the verbatim escalations below), but the shared single file `analog-switch.ts` cannot be committed clean per-recon because `git add` of that file would stage the un-scoped MODEINITFLOAT internalerror hunk (lines 199-202) and its banned `v41` historical-narrative comment (line 201) into the recon commit.

**Common blocker (all three recons):** the worktree diff of `analog-switch.ts` contains an out-of-scope behavioral hunk at the `swLoadHandles` MODEINITFLOAT branch (lines 199-202): the prior silent `current_state = HYST_OFF` fallback was replaced with `throw new Error("bad value for previous state in swload")` porting swload.c:90 internalerror. This hunk is NOT covered by acLoad/trunc/icParam (those govern stampAc, getLteTimestep, and IC-keyword plumbing respectively; the MODEINITFLOAT internalerror port is a separate state-machine hunk). It adds logic, is not structurally forced (compiles without it), and its comment (line 201) contains the banned historical-narrative term `v41`, violating the Code-Comments-No-Historical-Narrative rule. **Resolution needed from user/orchestrator:** either (a) attribute the swload.c:90 internalerror port to its own MODEINITFLOAT hunk/recon and strip the `v41` comment term, or (b) authorize folding it as in-blast-radius latent-bug. Either way the banned `v41` comment must be removed before any clean commit.

### sw#recon/acLoad — VERDICT: ESCALATE
stampAc (SPST :326-344, SPDT :530-553) is bit-isomorphic to swacload.c:26-33: (int)CKTstate0[SWswitchstate] => (states[0][base+SLOT_STATE])|0 with SWswitchstate=SWstate+0 (swdefs.h:60) => SLOT_STATE=0; g_now=current_state?SWonConduct:SWoffConduct truthiness => current_state?1/_rOn:1/_rOff; four real-part stamps +PP -PN -NP +NN via stampElement (swacload.c:30-33); SPDT runs body per path (base+0, base+2). #34 floor removal (Math.max(_rOn,1e-3)/Math.max(_rOff,_rOn*2)) is correctly removed from stampAc AND from load()+getPinCurrents per acLoad acceptance-criterion #3 — in-scope. All swsetup.c:50-53 TSTALLOC handle order, swdefs.h/sw.c citations independently verified against ref/ngspice. The recon body itself is APPLIED-quality. BLOCKER: the worktree diff of analog-switch.ts ALSO contains an out-of-scope behavioral hunk at swLoadHandles MODEINITFLOAT branch (lines 199-202): the prior silent `current_state = HYST_OFF` fallback was replaced with `throw new Error("bad value for previous state in swload")` porting swload.c:90 internalerror. This hunk is NOT covered by acLoad/trunc/icParam (those govern stampAc, getLteTimestep, and IC-keyword plumbing respectively; the MODEINITFLOAT internalerror port is a separate state-machine hunk). It adds logic, is not structurally forced (compiles without it), and its comment (line 201) contains the banned historical-narrative term `v41` ("the message is the v41 wording"), violating the Code-Comments-No-Historical-Narrative rule that every recon spec explicitly invokes. Because all three recons share the single file analog-switch.ts, `git add analog-switch.ts` would stage this un-scoped hunk (and banned comment) into the recon commit — the commit cannot be made clean per-recon. Resolution needed from user/orchestrator: either (a) attribute the swload.c:90 internalerror port to its own MODEINITFLOAT hunk/recon and strip the `v41` comment term, or (b) authorize folding it as in-blast-radius latent-bug. Either way the banned `v41` comment must be removed before commit.
- **Resolution:** _pending_

### sw#recon/trunc — VERDICT: ESCALATE
getLteTimestep (SPST :346-386) and _truncPath/getLteTimestep (SPDT :555-606) are bit-isomorphic to swtrunc.c:23-46: lastChange=ctrl0-ctrl1 reads SWctrlvalue (SLOT_V_CTRL=1, swdefs.h:61) from s0/s1 (swtrunc.c:23-25); off branch tests s0[base+SLOT_STATE]===0 literal (swtrunc.c:26), ref=vTh+vHys (:27), guard ctrl0<ref && lastChange>0 (:28), maxChange=(ref-ctrl0)*0.75+0.05 (:29-31), maxStep=maxChange/lastChange*deltaOld[0] left-to-right (:32); on branch ref=vTh-vHys (:37), ctrl0>ref && lastChange<0 (:38), *0.75-0.05 (:39-41); fall-through return Infinity (no *timeStep write); SPDT runs _truncPath on base+0 and base+2 returning Math.min (per-instance MIN fold, spec Part D, swtrunc.c:21-22). Operand order verbatim, no in-element clamp. The recon body itself is APPLIED-quality and unchanged in this diff (no trunc lines appear in `git diff`; the existing code already matches the spec's v41 baseline — but the diff is NON-empty for other recons' edits so this is not an empty-diff case). BLOCKER: same shared-file issue as acLoad — analog-switch.ts carries an out-of-scope behavioral hunk at MODEINITFLOAT (lines 199-202, swload.c:90 internalerror port) with a banned `v41` comment term (line 201), not covered by trunc's scope. Committing trunc's tsFile would stage that un-scoped hunk + banned comment. Escalate for the same resolution as acLoad before any commit.
- **Resolution:** _pending_

### sw#recon/icParam — VERDICT: ESCALATE
IC-keyword plumbing is bit-isomorphic to swparam.c/sw.c/swload.c/swdefs.h: `on`/`off` instance params default 0 (ANALOG_SWITCH_PARAM_DEFS :77-85) mapping IP("on",SW_IC_ON,IF_FLAG)/IP("off",SW_IC_OFF,IF_FLAG) (sw.c:13-14), SW_IC_ON=1/SW_IC_OFF=2 (swdefs.h:88-89); _zeroStateGiven field (SPST :276, SPDT :454) maps unsigned SWzero_stateGiven:1 (swdefs.h:46); constructor seeds false then applies on(=>true) then off(=>false) in keyword order (SPST :296-298, SPDT :479-481) per swparam.c:21-28; setParam `on`=>if(value)true / `off`=>if(value)false with the if(value->iValue) guard preserved (SPST :408-411, SPDT :630-633); SPST load() threads this._zeroStateGiven (:320); SPDT threads it into COM-NO path and false into COM-NC inverted-polarity path (:510,:524); ANALOG_SWITCH_ATTRIBUTE_MAPPINGS adds on/off modelParam:true rows (:836-838); MODEINITFIX|MODEINITJCT consumer branch (:160-175) is the line-for-line swload.c:43-60 port. The recon body itself is APPLIED-quality. BLOCKER: same shared-file issue — the worktree diff of analog-switch.ts contains an out-of-scope behavioral hunk at the MODEINITFLOAT branch (lines 199-202): swload.c:90 internalerror port (silent HYST_OFF fallback -> throw), NOT in icParam scope (icParam covers the MODEINITFIX|MODEINITJCT branch and IC plumbing, not MODEINITFLOAT), adds logic, not structurally forced, and carries a banned `v41` comment term (line 201). git add of icParam's tsFile stages this un-scoped hunk + banned comment into the commit. Escalate: attribute the swload.c:90 internalerror to its own MODEINITFLOAT hunk/recon (or authorize fold-in) AND strip the `v41` comment before any clean commit.
- **Resolution:** _pending_

## bjt (2026-06-03)

GATE+MERGE+TEARDOWN for unit `bjt` did NOT merge (non-isomorphic; gate/compile/merge skipped, gatePass=null). Rebase of worktree `.wt/bjt` onto `v41-port` succeeded clean (rebaseClean=true; no content conflict with the units merged since this worktree branched). MAIN src/ is clean (no leak). Kept work preserved to `C:/local_working_projects/digital_in_browser/.wt-failed/bjt.diff` (worktree diff vs v41-port, src/ only, 22289 bytes) for resume-from-patch. The 12 escalations passed to this gate (bjtacld.c::BJTacLoad, bjtconv.c::BJTconvTest, bjtdefs.h::sBJTinstance, bjtdefs.h::BJTstate-macros, bjtdefs.h::sBJTmodel, bjtload.c::BJTload part-1 / next1 / next2 / load:, bjtsetup.c::BJTsetup, bjttemp.c::BJTtemp) ALL carry the same `ESCALATE` verdict with the SAME root cause and route to **the existing live escalation `ESC-BJT-QS-collCXnode`** (this file lines 1099-1123, Resolution: _pending_) — they are cross-group architecture escalations (VERIFICATION.md §6), NOT closeable NUMERICAL fixture bugs (worktree diff EMPTY for these hunks; no firstDivergence / matrix-cell / step-iter signal since the non-isomorphic gate was skipped). Routing therefore is ESCALATIONS.md, not fix-list-phase-2-audit.md.

The v41 Kull quasi-saturation subsystem these 12 groups implement requires a new internal MNA node `BJTcollCXNode` whose allocation ORDER is owned by `compiler.ts` (`expandCompositeInstance`) + the device `setup()` TSTALLOC sequence — per CLAUDE.md "Sparse Solver is Settled" the exact upstream surfaces governing matrix shape/bit-exact parity — plus 9 new state slots (vbcx/vrci/irci/irci_Vrci/irci_Vbci/irci_Vbcx/qbcx/cqbcx/gbcx, BJTnumStates 24->33), 5 new matrix-pointer cells (BJTcollCX*) with the BJTcolColPrimePtr->BJTcollCollCXPtr / BJTcolPrimeColPtr->BJTcollCXCollPtr renames, the QS model params (RCO/VO/GAMMA/QCO -> intCollResist/epiSatVoltage/epiDoping/epiCharge) + BEsatCur/BCsatCur dual-satcur split, the third-junction vbcx/delvbcx in BJTconvTest, and the QS temperature block in `computeBjtTempParams` (bjttemp helper, out of this unit's tsFile). digiTS `bjt.ts` is the v26 baseline (Grep: 0 matches for collCX/intCollResist/epiSatVoltage/epiDoping/irci/vbcx/quasimod). Blast radius = `bjt.ts` + `compiler.ts` (node allocation) + `computeBjtTempParams` — beyond the bjt.ts functionGroup, hence ESCALATE per VERIFICATION.md §6 + device-class-scope.md (BJT is IN -> the QS sub-feature is a required reconstruction item, not an accepted divergence; ruling it out is a user-only amendment to device-class-scope.md). Per-group verbatim verdicts (area-scaling refactor, IBE/IBC split, subSatCurGiven substrate conditional in-file MISMATCH vs the cross-group node-allocation blocker) are recorded under ESC-BJT-QS-collCXnode and its 2026-06-03 verifier addendum (lines 1116-1123).
...
### sw#recon/acLoad gate — REBASE BLOCKED by foreign dirty working tree (sequencing)

- **source=sw GATE+MERGE rebase step | verdict=REBASE NOT CLEAN (dirty-tree precondition, NOT a conflict and NOT a numerical divergence).** `git -C .wt/sw rebase v41-port` aborts before starting with `error: cannot rebase: You have unstaged changes. Please commit or stash them.` This is a workflow/sequencing blocker, not a `firstDivergence` / matrix-cell / step-iter gate failure, so it is recorded here rather than in `spec/fix-list-phase-2-audit.md`.
- **What the dirty tree contains.** `git -C .wt/sw status --porcelain` reports:
  - ` M src/components/active/cccs.ts` — **foreign to the sw unit.** The sw unit's two committed commits (`3ac5ed8b` recon acLoad + `8eb4e886` SWload internalerror fall-through) touch **only** `src/components/active/analog-switch.ts` (`git diff --name-only f4f5366e..HEAD` = `analog-switch.ts` alone). The `cccs.ts` edit is uncommitted working-tree state for the **CCCS** device, unrelated to the analog switch.
  - ` M rename-maps/sw.md` — uncommitted edits to the sw rename map (adds `internalerror`, `current_state` init, and a `CKTtroubleElt` "no LoadContext counterpart — ESCALATED" row).
  - `?? .tsc-out.txt` — stray untracked build artifact.
- **Why this is a STOP, not a force-through.** The `cccs.ts` working-tree diff **reverts** the previously-committed ngspice-correct CCCS M-fold implementation (removes `_effectiveCoeff()` / `_gain` / `_mValue` / `_mGiven` / `_gainGiven` / `_linearDefault` and the `cccspar.c:25-28` hot-loadable gain-coefficient path) back to a baked-at-construction `currentGain * M` inline-gain form. Clearing it via `git stash` / `git checkout -- cccs.ts` / `git reset --hard` would silently destroy that work — which is forbidden (CLAUDE.md "Do not overwrite working-tree changes. Other agents may be working in parallel."), and `.wt/sw` is one of nine live worktrees. The agent therefore did **not** run any destructive cleanup and did **not** rebase. `rebaseClean=false`, gate never reached (`gatePass=null`), unit not merged.
- **What is blocked:** the entire sw unit merge (`sw#recon/acLoad`, hunkHash `24b269993296d2ea`, ref `harvested:3ac5ed8b`). Its committed work is sound and isolated to `analog-switch.ts`; only the foreign dirty tree blocks the rebase. MAIN's `src/` stayed clean (`mainSrcClean=true`). Maximal kept work preserved at `C:/local_working_projects/digital_in_browser/.wt-failed/sw.diff` (`git diff v41-port -- src/`, sw worktree) for resume-from-patch.
- **Decision needed from user:** (1) adjudicate the uncommitted `cccs.ts` revert — is it intended (a CCCS unit's in-flight edit that wrongly landed in `.wt/sw`) or stray noise to discard? It reverts an ngspice-correct M-fold path, so it should not be silently committed or silently dropped. (2) Once the `.wt/sw` working tree is clean (foreign `cccs.ts` / `rename-maps/sw.md` edits committed to their proper unit or removed by the user), re-run the sw GATE+MERGE unit so the rebase onto `v41-port`, the compile gate, and the `sw-gate.dts` harness/self-compare gate can proceed.
- **Resolution:** _pending_
...
### `analysis#recon/cshunt` — implementation is bit-exact CORRECT vs ngspice, but the governing spec is NOT RATIFIED and its prose CONFLICTS with ngspice ground truth (spec-authoring item, not numerical)

- **source=analysis#recon/cshunt | verdict=ESCALATE.** The cshunt reconstruction was built and is bit-exact correct against the ngspice DLL, but the governing spec cannot be cleanly APPLIED for two reasons — the recon was left PENDING, no `progress.json` APPLIED flip, no commit. Routed here (not `fix-list-phase-2-audit.md`) because the implementation is bit-exact correct; the defect is in the spec text + ratification status, an architectural/spec-authoring item for the user/ledger-owner.

  Full note (verbatim):

  > Implementation is bit-exact CORRECT against ngspice ground truth, but the governing spec cannot be cleanly APPLIED for two reasons. (1) SPEC NOT RATIFIED: spec/v41-port/reconstruction/analysis-cshunt.md status line reads 'PENDING — authored 2026-06-05, awaiting user ratification' (cf. analysis-tf.md which is RATIFIED). cshunt has NO entry in progress.json. (2) SPEC-vs-SOURCE CONFLICT: the spec's Part C, identifier map, and acceptance criterion #4 mandate that the injection cover EVERY non-ground voltage node INCLUDING device-internal voltage nodes (BJT internal C/B/E, diode internal anode). This is FACTUALLY WRONG. Independent re-derivation from ref/ngspice/src/frontend/spiceif.c:95-181 (if_inpdeck): the call order is INPpas2 (:168, builds CKTnodes from netlist instance lines) -> INPpas4 (:177, the cshunt injection) -> INPpas3 (:181), then if_inpdeck RETURNS. CKTsetup (where DIOsetup mints the internal node via CKTmkVolt(...,'internal'), ref/ngspice/src/spicelib/devices/dio/diosetup.c:312) runs LATER and SEPARATELY (spiceif.c:1394 / cktdojob.c), AFTER INPpas4. Therefore device-internal nodes do NOT exist at INPpas4 time and get NO shunt cap. The BUILDER correctly built to ngspice ground truth: src/solver/analog/analog-engine.ts _injectCshuntCapacitors iterates only cac.nodeCount (external/netlist + composite-internal nodes), explicitly excluding per-device-internal nodes — the OPPOSITE of what the spec's prose demands, and the CORRECT behavior. EVIDENCE the implementation is right: src/solver/analog/__tests__/ngspice-parity/cshunt-parity.test.ts passes 2/2 bit-exact vs the ngspice DLL (cshunt=1e-9 injection matches ngspice INPpas4 every NR iteration on cshunt-gate.dts which has a diode internal node; and cshunt-unset matches the no-cshunt baseline). MCP harness on cshunt-gate.dts (no-cshunt baseline) also shows firstDivergence null across all classes. FILE-SCOPE clean: diff touches src/core/analog-engine-interface.ts (SimulationParams.cshunt + DEFAULT -1, in scope), src/solver/analog/analog-engine.ts (injection pass, in scope), and src/solver/analog/__tests__/harness/comparison-session.ts (Part-E _materializeCir .options cshunt card — structurally forced for the gate, minimal one-line splice, no new logic). capacitor.ts (in cshunt tsFiles) is correctly UNMODIFIED (spec mandates reusing CapacitorElement verbatim). RESOLUTION (user action): correct the cshunt spec's Part C, identifier-map rows ('ckt->CKTnodes post-INPpas2 incl. device-internal' and 'device-internal nodes are included'), and acceptance criterion #4 to match ngspice ground truth (INPpas4 runs before CKTsetup => external/netlist voltage nodes ONLY, device-internal nodes excluded), then RATIFY the spec. After that correction+ratification this exact implementation is a clean MATCH. Left PENDING, no progress.json APPLIED flip, no commit. Not filed to fix-list-phase-2-audit.md because that file is for numerical bugs and this implementation is bit-exact correct — the defect is in the spec text + ratification status, an architectural/spec-authoring item for the user/ledger-owner, not a numerical fix.

- **Relation to ESC-024:** ESC-024 (`cshunt` — unported behavioral param) framed cshunt as needing a new `SimulationParams` field + load-path shunt conductance. That work is now DONE and verified bit-exact; this entry supersedes ESC-024's framing — the remaining blocker is purely the spec's ratification status + the Part-C/criterion-#4 prose conflict with ngspice ground truth.
- **Decision needed from user:** correct `spec/v41-port/reconstruction/analysis-cshunt.md` Part C / identifier-map / acceptance criterion #4 so the injection scope reads "external/netlist voltage nodes ONLY, device-internal nodes excluded" (INPpas4 runs before CKTsetup), then set its Status line to RATIFIED and add the `analysis#recon/cshunt` entry to `progress.json`. After that the existing implementation (preserved at `.wt-failed/analysis.diff` for resume) is a clean MATCH and can be recorded APPLIED.
- **Resolution:** _pending_

### `analysis/dctran.c::DCtran` — dctran.c hunk-group INCOMPLETE: h006 (transient-OP → first-step state copy) received NO disposition; group cannot be APPLIED until h006 recorded (unmapped-hunk / group-incompleteness, not numerical)

- **source=analysis/analysis/dctran.c::DCtran(CKTcircuit *ckt, | verdict=3-ROUNDS-EXHAUSTED.** Routed here (not `fix-list-phase-2-audit.md`) because there is no firstDivergence, no matrix/Jacobian cell, no absDelta, no step/iter bit-exact gap, and no fixture GATE-FAIL — the defect is that a hunk group is not fully dispositioned (one hunk omitted from `progress.json`), an unmapped-hunk / group-incompleteness item for the ledger owner.

  Full note (verbatim):

  > Group is INCOMPLETE: of the 6 hunks (h004-h009), the applier dispositioned only 5 (h004,h005,h007,h008,h009 -> ESCALATED TARGET-ABSENT in progress.json + ESCALATIONS.md). Hunk h006 received NO disposition at all: no progress.json entry (keys present are h001,h002,h004,h005,h007,h008,h009,h011-h014; h006 absent), no escalation, no edit. h006 (diffDoc spec/ngspice-v41-engine-diffs/analysis.md:3989-3990, ngspice ref/ngspice/src/spicelib/analysis/dctran.c:325-326, @@ -340,7 +316,7 @@) is the transient-OP-to-first-step state copy `bcopy(CKTstate0,CKTstate1,...)` -> `memcpy(CKTstate1,CKTstate0,...)` (both copy state0->state1). This is a real numerical state operation, NOT target-absent: the engine already realizes it at C:/local_working_projects/digital_in_browser/.wt/analysis/src/solver/analog/analog-engine.ts:2261 `cac.statePool.states[1].set(cac.statePool.states[0]);` (dst=states[1], src=states[0]), with the cite comment at lines 2256 ('dctran.c:349-350- memcpy(CKTstate1, CKTstate0, numStates*sizeof(double))'). That line is line-isomorphic to the post-image memcpy. REQUIRED FIX: record analysis/dctran.c#h006 as APPLIED in progress.json (hunkHash + evidence analog-engine.ts:2261), then commit. The 5 escalations themselves are independently verified correct against the ngspice source (h004 dctran.c:235-255 stdout fprintf console block; h005 dctran.c:257-260 SPfrontEnd->OUTendPlot; h007 dctran.c:419 #ifdef XSPICE g_ipc/wantevtdata; h008 dctran.c:445-451 #ifdef XSPICE ipc_send_data; h009 dctran.c:468-470 CKTdump output gate) — all are genuine console/plot/XSPICE-IPC/output-capture constructs absent from the engine-agnostic engine, consistent with already-frozen NO-COUNTERPART h003/h010. But because the group as a whole is not fully dispositioned (h006 omitted), the group cannot be APPLIED; sent back to be completed. File-scope: worktree diff touches only spec/v41-port/progress.json + ESCALATIONS.md (no stray src files); the named tsFile src/solver/analog/analog-engine.ts shows NO change, which is correct for an all-escalation+already-present-counterpart group but means h006's APPLIED record is still owed.
...
#### Teardown disposition (2026-06-05, WORKTREE GATE+MERGE+TEARDOWN)
Worktree `.wt/dio` passed the compile gate (28/28: compile-analog-partition + compiler tests) and fast-forward merged into `v41-port`; MAIN `src/` clean post-merge. The dioconv functionGroup splits as: **h001 / h003 APPLIED** — h001 = local-var decls `delTemp` / `deldelTemp` in the convergence test; h003 = accessor rename — both line-isomorphic and landed in `diode.ts`. The convergence math is verified **bijective**: `checkConvergence` (`src/components/semiconductors/diode.ts:1306-1327`) mirrors `dioconv.c` — `vd` from `rhsOld`, `delvd`, self-heating `delTemp` / `deldelTemp`, `cdhat = id + gd*delvd + dIdio_dT*deldelTemp`, `tol = reltol*max(|cdhat|,|id|) + iabstol`. **h002 stays ESCALATED** (out-of-group coupling): the device-completion harness RED on `diode-canon-cap-rc.dts` traces to the engine NR iteration-count gate in `newton-raphson.ts:771-775` (the niiter.c `CKTnoncon` / `iterno!=1` port), outside dio recon scope (`diode.ts`). The faithful fix provably requires the named out-of-scope engine file, so this remains a legitimate FILE-SCOPE escalation routed to the engine unit (decision-needed above). The device-harness gate was NOT run this teardown pass (out of verifier scope).
